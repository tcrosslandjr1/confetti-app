...
function AdultLifeHubSections({ onPickPortal, onOpenCommand }) {
  ...
  const [picked, setPicked] = useStateAd([]);  // multi-select array of portal labels
      ...
      {ADULT_SECTIONS
        ...
        .map(section => (
          <AdultSection key={section.id}
                        ...
                        picked={picked}
                        onPick={(p, s) => togglePick(p.label)} />
      ))}
      {picked.length > 0 && (
        <div style={{
          position: 'sticky', bottom: 16, zIndex: 10,
          marginTop: 14, marginInline: -22, padding: '12px 22px',
        }}>
          <button onClick={() => onPickPortal({ label: picked.join(' + ') })} style={{
            appearance: 'none', cursor: 'pointer', width: '100%',
            display: 'flex', alignItems: 'center', gap: 12,
            padding: '14px 18px',
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              }}>{picked.length} PICKED · MULTI-PLAN</div>
              <div style={{
                fontFamily: 'var(--cf-display)', fontWeight: 900, fontSize: 16,
                letterSpacing: '-0.025em', marginTop: 1,
                whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
              }}>build with {picked.join(' · ')}</div>
            </div>
...
function AdultSection({ section, focused, onPick, picked = [] }) {
