...
function SignUpScreen({ onDone, onSignIn }) {
  ...
  const next = () => {
    if (step === 0) {
      ...
      if (!(emailOk && phoneOk && nameOk)) return;
    }
    if (step === 1 && !data.city) return;
    if (step === 2 && Object.keys(data.vibeRanks || {}).length === 0) return;
    step < steps.length - 1 ? setStep(step + 1) : onDone(data);
  };
...
function SignupWho({ data, setData }) {
  ...
  const showEmail = (data.email || '').length > 0;
  const showPhone = (data.phone || '').length > 0;
  const showName = (data.name || '').length > 0;
  ...
  return (
    <>
      ...
      <ValidatedField label="email · for sign-in + magic link"
        placeholder="jess@brooklyn.com"
        value={data.email || ''}
        onChange={v => setData({ ...data, email: v })}
        valid={emailValid} show={showEmail}
        error="that doesn't look like an email" />

      <ValidatedField label="name"
        placeholder="jess s."
        value={data.name || ''}
        onChange={v => setData({ ...data, name: v })}
        valid={nameValid} show={showName}
        error="add at least 2 characters" />

      <ValidatedField label="phone · so crew can find you"
        placeholder="(347) 555-0182"
        value={data.phone || ''}
        onChange={v => setData({ ...data, phone: v })}
        valid={phoneValid} show={showPhone}
        error="need a 10-digit number"
        sub={phoneValid ? '✓ we\'ll send a one-time code on the next step' : null} />

      {/* Validation summary */}
      <div style={{
        padding: 10,
        background: allValid ? 'var(--accent-4, #2bb673)' : 'var(--paper)',
        color: allValid ? 'var(--paper)' : 'var(--ink)',
        border: '2px solid var(--ink)', borderRadius: 10,
        fontFamily: 'var(--cf-mono)', fontSize: 10, fontWeight: 700,
        letterSpacing: '.06em', lineHeight: 1.5,
      }}>
        {allValid
          ? '✓ ALL GOOD · TAP NEXT TO PICK YOUR CITY'
          : `${[nameValid, emailValid, phoneValid].filter(Boolean).length}/3 FIELDS DONE · FIX RED ONES BEFORE NEXT`}
      </div>
...
function ValidatedField({ label, placeholder, value, onChange, valid, show, error, sub }) {
  const showError = show && !valid;
  const showSuccess = show && valid;
  return (
    <div>
      <div style={{
        fontFamily: 'var(--cf-mono)', fontSize: 10, fontWeight: 800,
        ...
        {label}
        {showSuccess && <span style={{
          color: 'var(--accent-4, #2bb673)', fontSize: 12, fontWeight: 900,
        }}>✓</span>}
      </div>
      <input value={value} onChange={e => onChange(e.target.value)}
             ...
             style={{
               ...
               border: `2.5px solid ${showError ? '#d32323' : 'var(--ink)'}`,
               borderRadius: 14,
               background: 'var(--paper)',
               fontFamily: 'var(--cf-ui)', fontSize: 16, fontWeight: 700,
               color: 'var(--ink)', outline: 'none',
               boxShadow: `4px 4px 0 ${showError ? '#d32323' : 'var(--ink)'}`,
      ...
      {showError && (
        <div style={{
          marginTop: 5,
          fontFamily: 'var(--cf-mono)', fontSize: 10, fontWeight: 800,
          color: '#d32323', letterSpacing: '.04em',
        }}>⚠ {error}</div>
      )}
      {sub && showSuccess && (
        <div style={{
          marginTop: 5,
          fontFamily: 'var(--cf-mono)', fontSize: 10, fontWeight: 700,
          color: 'var(--accent-4, #2bb673)', letterSpacing: '.04em',
        }}>{sub}</div>
      )}
    </div>
  );
