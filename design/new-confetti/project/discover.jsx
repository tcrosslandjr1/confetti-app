...
const REELS = [
  { who: 'maya', venue: 'Westlight rooftop', stop: 1, vibe: 'romantic',
    ...
    stops: [
      { name: 'Westlight rooftop', tag: 'rooftop · $$$', color: 'var(--accent-1)', cost: '$48' },
      { name: 'Roebling Tea Room', tag: 'date dinner', color: 'var(--accent-2)', cost: '$72' },
      { name: 'Skinny Dennis', tag: 'dive nightcap', color: 'var(--accent-3)', cost: '$22' },
    ] },
  { who: 'devon', venue: 'Quartz Room show', stop: 3, vibe: 'hype',
    caption: 'pearl charles is unreal live', color: 'var(--accent-3)',
    likes: 89, comments: 6,
    stops: [
      { name: 'Threes Brewing', tag: 'beer warm-up', color: 'var(--accent-2)', cost: '$18' },
      { name: 'Oxomoco', tag: 'tacos · loud', color: 'var(--accent-1)', cost: '$58' },
      { name: 'Quartz Room', tag: 'live show', color: 'var(--accent-3)', cost: '$26' },
    ] },
  { who: 'sam', venue: 'Lupa Notte', stop: 2, vibe: 'foodie',
    caption: 'this carbonara healed me', color: 'var(--accent-2)',
    likes: 312, comments: 42,
    stops: [
      { name: "Skinny Pete's", tag: 'dive warm-up', color: 'var(--accent-2)', cost: '$14' },
      { name: 'Lupa Notte', tag: 'italian · ★ counter', color: 'var(--accent-1)', cost: '$52' },
      { name: 'Eavesdrop', tag: 'jazz nightcap', color: 'var(--accent-3)', cost: '$32' },
    ] },
];

function ReelsScreen({ onBack, onAddStop }) {
  const [idx, setIdx] = useStateD(0);
  const onScroll = (e) => {
    const top = e.target.scrollTop;
    const h = e.target.clientHeight;
  ...
  return (
    <div style={{
      ...
      <div onScroll={onScroll}
        ...
        {REELS.map((r, i) => (
          <ReelCard key={i} reel={r} active={i === idx} onAddStop={onAddStop} />
        ))}
      </div>
    </div>
  );
...
function ReelCard({ reel, active, onAddStop }) {
  const [addedIds, setAddedIds] = useStateD(new Set());
  const addStop = (s) => {
    onAddStop?.(s);
    setAddedIds(prev => new Set([...prev, s.name]));
  };
  return (
    <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            {reel.stops && (
              <div style={{ marginTop: 10 }}>
                ...
                }}>or grab just one stop</div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {reel.stops.map((s, i) => {
                    const added = addedIds.has(s.name);
                    return (
                      <button key={i} onClick={() => addStop(s)} disabled={added}
                        style={{
                          appearance: 'none', cursor: added ? 'default' : 'pointer',
                          display: 'inline-flex', alignItems: 'center', gap: 5,
                          padding: '5px 9px 5px 5px',
                          border: '2px solid var(--paper)', borderRadius: 999,
                          background: added ? 'var(--accent-4, #2bb673)' : 'rgba(0,0,0,0.45)',
                          color: 'var(--paper)',
                          fontFamily: 'var(--cf-ui)', fontSize: 11, fontWeight: 800,
                          backdropFilter: 'blur(10px)',
                        }}>
...
function CrewMapScreen({ onBack }) {
  ...
  const crew = [
    { who: 'maya',  x: 28 + Math.sin(tick * 0.4) * 3, y: 22,
      c: 'var(--accent-1)', status: 'at Westlight', dist: '1.2 mi', eta: 'arrived', mode: '🚶' },
    { who: 'devon', x: 70, y: 38 + Math.cos(tick * 0.3) * 2,
      c: 'var(--accent-3)', status: 'on L train', dist: '0.8 mi', eta: '6 min', mode: '🚇' },
    { who: 'sam',   x: 56, y: 60, c: 'var(--accent-2)',
      status: 'at Lupa', dist: '0.3 mi', eta: 'arrived', mode: '🚶' },
    { who: 'me',    x: 44, y: 48, c: 'var(--paper)', isMe: true,
      status: 'here', dist: '—', eta: '—', mode: '📍' },
  ];
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ flex: 1, display: 'flex', gap: 6 }}>
            ...
            }}><span>U</span>connect Uber</button>
            <button style={{
              flex: 1, appearance: 'none', cursor: 'pointer',
              padding: '6px 8px',
              border: '2px solid var(--ink)', borderRadius: 8,
              background: '#FF00BF', color: '#fff',
              fontFamily: 'var(--cf-ui)', fontSize: 10, fontWeight: 800,
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 5,
            }}><span>L</span>connect Lyft</button>
          </div>
        </div>
