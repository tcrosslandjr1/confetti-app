...
// ═════════════════════════════════════════════════════════════════════
const CHAT_SCRIPT = [
  { from: 'bot', text: "hey, jess. i'm sparkle — confetti's planner. what are you up for?", chips: ['date night', 'family park day', 'cookout for 8', 'kids birthday', 'tourist day in NYC', 'crew of 6'] },
  ...
  { from: 'bot', text: "perfect. 3 stops, 4.5 hrs, ~$88. shall i build it?", chips: ['build my plan', 'tweak it', 'try again'] },
];

function ChatScreen({ onPlan, onBack, seedIntent }) {
  const [step, setStep] = useStateX(1);   // how many scripted messages shown
  const [typing, setTyping] = useStateX(false);
  const [input, setInput] = useStateX(seedIntent || '');
  const [extra, setExtra] = useStateX([]); // free-text messages beyond script
  const scrollRef = useRefX(null);

  // Auto-seed when user lands here from a portal pick
  useEffectX(() => {
    if (!seedIntent) return;
    setExtra([{ from: 'user', text: seedIntent }]);
    setInput('');
    (async () => {
      setTyping(true);
      try {
        const reply = await window.claude.complete({
          messages: [{
            role: 'user',
            content: `You are Sparkle. The user just tapped the "${seedIntent}" portal. Reply in one warm, lowercase, casual sentence under 25 words. Confirm you heard them and ask ONE smart follow-up (city, budget, or group size).`
          }]
        });
        setExtra(e => [...e, { from: 'bot', text: (reply || '').trim() }]);
      } catch {
        setExtra(e => [...e, { from: 'bot', text: `${seedIntent} — got it. what's the city + headcount?` }]);
      } finally {
        setTyping(false);
      }
    })();
  ...
  const handleChip = (chip) => {
    if (chip === 'build my plan' || chip === 'print it') return onPlan('next');
  ...
  const sendInput = async () => {
    ...
    try {
      ...
      const reply = await window.claude.complete({
        messages: [{
          role: 'user',
          content: `You are Sparkle, Confetti's planning agent. You help plan nights out, family days, parties, BBQs, tourist days. You're warm, casual, lowercase, brief — under 30 words per reply. Ask ONE smart follow-up at a time. Once you have enough (vibe + location + size + budget), say "i can build this. ready?" and stop asking.

USER CONTEXT (already known — don't ask for these):
- city: Washington DC (default location from their profile)
- name: jess
- prior passes: 18
...
If user message includes group size of 3+, casually mention you can auto-set up a group text with their crew once they confirm.

Respond as Sparkle. After your message, on a NEW LINE add exactly: SUGGEST: [3-4 lowercase short quick-reply options the user might tap, comma-separated, no quotes].
Example:
...
SUGGEST: under $40, $40-80, $80-150, no cap`,
      ...
      const suggestMatch = raw.match(/SUGGEST:\s*(.+)$/im);
      const text = raw.replace(/SUGGEST:.+$/im, '').trim();
      const suggested = suggestMatch
        ? suggestMatch[1].split(',').map(s => s.trim()).filter(Boolean).slice(0, 4)
        : null;
      const wantsPrint = /ready\?|build it|i can build/i.test(text);
      setExtra(e => [...e, {
        from: 'bot', text,
        chips: wantsPrint ? ['build my plan', 'tweak it', 'one more thing']
               : suggested,
      }]);
    } catch (e) {
        ...
        <div style={{
          ...
          {last.chips.map(c => (
            <button key={c} onClick={() => handleChip(c)} style={{
              ...
              padding: '8px 14px',
              border: '2.5px solid var(--ink)', borderRadius: 999,
              background: (c === 'build my plan' || c === 'print it') ? 'var(--accent-1)' : 'var(--paper)',
              fontFamily: 'var(--cf-ui)', fontSize: 13, fontWeight: 800,
              color: 'var(--ink)',
              boxShadow: (c === 'build my plan' || c === 'print it') ? '3px 3px 0 var(--ink)' : 'none',
            }}>{c}</button>
          ))}
        </div>
      )}
