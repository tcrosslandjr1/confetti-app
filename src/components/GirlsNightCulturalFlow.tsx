import { Sparkles, Users, Globe2, Camera, Utensils, Music, PartyPopper } from "lucide-react";

/**
 * Girls Night — 4-step planning framework. Mirrors the agent prompt in
 * src/lib/agents/girls-night-presets.ts → buildGirlsNightCulturalPrompt()
 * and docs/agents/confetti-girls-night-cultural-framework.md.
 */
export function GirlsNightCulturalFlow() {
  return (
    <section className="border-b border-border bg-muted/30">
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-4 flex items-center gap-2">
          <Sparkles className="h-4 w-4 text-primary" />
          <span className="font-display text-sm font-bold">The best girls' night out plan</span>
        </div>

        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Step
            n={1}
            title="Know the group"
            icon={<Users className="h-4 w-4" />}
            rows={[
              ["Shared culture", "Build around that culture's food, music, social style."],
              ["Mixed culture", "Build around everyone contributing something."],
            ]}
          />
          <Step
            n={2}
            title="Pick the anchor"
            icon={<Utensils className="h-4 w-4" />}
            rows={[
              ["Food", "Shareable dinner"],
              ["Music", "Playlist or live DJ"],
              ["Activity", "Karaoke, dancing, lounge, bowling, paint-and-sip, comedy"],
            ]}
          />
          <Step
            n={3}
            title="Make it social-media friendly"
            icon={<Camera className="h-4 w-4" />}
            rows={[
              ["Fit check", "Outfit moment before you leave"],
              ["Group photo", "One real, one chaotic"],
              ["Short video", "Reel-worthy 7 seconds"],
              ["Dessert", "Or late-night food"],
            ]}
          />
          <Step
            n={4}
            title="Choose the final plan"
            icon={<PartyPopper className="h-4 w-4" />}
            rows={[
              ["Shared", "Cultural dinner + karaoke/dance/music lounge + dessert"],
              ["Mixed", "Culture-swap dinner + passport playlist + group-voted activity"],
            ]}
          />
        </div>
      </div>
    </section>
  );
}

function Step({
  n,
  title,
  icon,
  rows,
}: {
  n: number;
  title: string;
  icon: React.ReactNode;
  rows: [string, string][];
}) {
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="flex items-center gap-2">
        <span className="grid h-6 w-6 place-items-center rounded-full bg-primary text-[11px] font-bold text-primary-foreground">
          {n}
        </span>
        <div className="flex items-center gap-1.5 text-foreground">
          {icon}
          <span className="font-display text-sm font-bold">{title}</span>
        </div>
      </div>
      <ul className="mt-3 space-y-2">
        {rows.map(([k, v]) => (
          <li key={k} className="text-xs">
            <p className="font-semibold text-foreground">{k}</p>
            <p className="text-muted-foreground">{v}</p>
          </li>
        ))}
      </ul>
    </div>
  );
}
