...
import { Calendar, Clock, Send, Sparkles, Timer, X, AlertOctagon, CheckCircle2 } from "lucide-react";
...
export function LateRescheduleFab({ tripId, partyName, groupSize, stops }: Props) {
  ...
  function staggerVenueToasts(kind: "late" | "reschedule" | "cancel", buildMsg: (venue: string) => string) {
  ...
  function confirmLate() {
    ...
    const count = staggerVenueToasts("late", (v) => `Heads up — ${partyName} running ~${lateMin} min late. New ETA at ${v}: ${shiftTimeLabel(firstStop?.time ?? "12:00 PM", lateMin)}.`);
    setTimeout(() => {
      setSending(false);
      setMode(null);
  ...
  function confirmReschedule() {
    ...
    const pretty = new Date(iso).toLocaleString([], { weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" });
    const count = staggerVenueToasts("reschedule", (v) => `${partyName} rescheduled to ${pretty} at ${v}.`);
    setTimeout(() => {
      setSending(false);
      setMode(null);
  ...
  function confirmCancel() {
    setSending(true);
    setCancelled(tripId);
    const count = staggerVenueToasts("cancel", (v) => `${partyName} cancelled their reservation at ${v}. Apologies for the late notice.`);
    setTimeout(() => {
      setSending(false);
      setMode(null);
  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3 sm:bottom-8 sm:right-8">
          ...
          {mode === "menu" && (
            ...
            >
              {[
                ...
                { key: "reschedule", label: "Reschedule / Cancel", icon: Calendar, bg: "bg-sky-300" },
              ].map((opt, i) => (
                <motion.button
                  key={opt.key}
                  initial={{ opacity: 0, y: 16, scale: 0.85 }}
                  animate={{ opacity: 1, y: 0, scale: 1, transition: { delay: i * 0.05, type: "spring", stiffness: 360, damping: 22 } }}
        ...
        >
          {mode === "menu" ? <X className="h-7 w-7" strokeWidth={3} /> : <Clock className="h-7 w-7" strokeWidth={2.8} />}
        </motion.button>
      </div>
...
function SheetHeader({ title, subtitle, accent }: { title: string; subtitle: string; accent: string }) {
  return (
    <div className={`border-b-[3px] border-foreground ${accent} px-5 py-4`}>
      <p className="text-[11px] font-bold uppercase tracking-wider text-foreground/70">{subtitle}</p>
      <h2 className="mt-0.5 font-display text-xl font-bold text-foreground">{title}</h2>
    </div>
  );
}

function PartyPreview({ partyName, groupSize, originalTime, newEta }: { partyName: string; groupSize: number; originalTime: string; newEta?: string }) {
  return (
    <div className="rounded-2xl border-2 border-foreground bg-muted/40 p-4">
      ...
      <p className="text-xs text-muted-foreground">{groupSize} {groupSize === 1 ? "person" : "people"}</p>
      <div className="mt-3 flex items-center gap-3 text-sm">
        <span className="inline-flex items-center gap-1 rounded-md bg-card px-2 py-1 font-mono text-xs line-through opacity-70">{originalTime}</span>
        {newEta && (
          <>
            <span className="text-muted-foreground">→</span>
            <span className="inline-flex items-center gap-1 rounded-md bg-amber-300 px-2 py-1 font-mono text-xs font-bold text-foreground">{newEta}</span>
          </>
        )}
      </div>
    </div>
...
function LateSheet({
  partyName, groupSize, originalTime, newEta, lateMin, setLateMin, onConfirm, sending, venueCount,
}: {
  partyName: string; groupSize: number; originalTime: string; newEta: string;
  lateMin: number; setLateMin: (n: number) => void; onConfirm: () => void; sending: boolean; venueCount: number;
}) {
  return (
    <>
      ...
      <div className="space-y-4 p-5">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">How late?</p>
              ...
              return (
                <button
                  ...
                  className={`rounded-full border-2 border-foreground px-3.5 py-1.5 text-xs font-bold transition-all ${
                    active ? "bg-amber-300 shadow-[3px_3px_0_0_hsl(var(--foreground))]" : "bg-card hover:-translate-y-0.5"
                  }`}
                >
                  +{m === 60 ? "1 hr" : `${m} min`}
                </button>
        ...
        <PartyPreview partyName={partyName} groupSize={groupSize} originalTime={originalTime} newEta={newEta} />
...
function RescheduleMenu({ onPickDate, onCancel }: { onPickDate: () => void; onCancel: () => void }) {
  return (
    <>
      ...
      <div className="space-y-3 p-5">
        ...
        >
          ...
          <span className="flex-1">
            ...
            <span className="block text-[11px] text-foreground/70">Move the whole night to another slot</span>
          </span>
        </button>
        ...
        >
          ...
          <span className="flex-1">
            ...
            <span className="block text-[11px] text-foreground/70">Releases venues and notifies guests</span>
          </span>
        </button>
      </div>
    </>
...
function PickDateSheet({
  newDate, setNewDate, newTime, setNewTime, onBack, onConfirm, sending, partyName, groupSize,
}: {
  newDate: string; setNewDate: (s: string) => void;
  newTime: string; setNewTime: (s: string) => void;
  onBack: () => void; onConfirm: () => void; sending: boolean;
  partyName: string; groupSize: number;
}) {
      ...
      <div className="space-y-4 p-5">
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">New date</span>
            <input
              type="date"
              min={today}
              value={newDate}
          ...
          <label className="block">
            <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Start time</span>
            <input
              type="time"
              value={newTime}
        ...
        <div className="rounded-2xl border-2 border-foreground bg-muted/40 p-4">
          <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Will notify</p>
          <p className="mt-1 text-sm font-semibold">{partyName} · {groupSize} {groupSize === 1 ? "person" : "people"}</p>
          {newDate && (
        ...
        <div className="flex gap-2">
          ...
          >
            {sending ? (
              <>
                <motion.span animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="inline-block h-4 w-4 rounded-full border-2 border-foreground border-t-transparent" />
                Sending…
              </>
            ) : (
...
function CancelSheet({
  partyName, groupSize, venueCount, onBack, onConfirm, sending,
}: {
  partyName: string; groupSize: number; venueCount: number;
  onBack: () => void; onConfirm: () => void; sending: boolean;
}) {
  return (
    <>
        ...
        <div className="rounded-2xl border-2 border-foreground bg-rose-50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm">
              <p className="font-bold">{partyName} · {groupSize} {groupSize === 1 ? "person" : "people"}</p>
              <p className="mt-1 text-foreground/70">
                Cancellation emails go to all <strong>{venueCount}</strong> venue{venueCount === 1 ? "" : "s"} and your guests.
                The plan will show a "Cancelled" overlay.
              </p>
            </div>
          </div>
        </div>
