...
const META: Record<ViralTag, { label: string; icon: typeof Flame; className: string }> = {
  ...
  instagrammable: {
    ...
    icon: Camera,
    className: "bg-fuchsia-500/15 text-fuchsia-600 border-fuchsia-500/30",
  },
  hidden_gem: {
    label: "Hidden Gem",
    icon: Sparkles,
...
export function tagLabel(tag: ViralTag) {
  return META[tag]?.label ?? tag;
}
...
export function ViralTagChip({ tag, size = "sm" }: { tag: ViralTag; size?: "sm" | "md" }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full border font-semibold ${padding} ${m.className}`}
    >
