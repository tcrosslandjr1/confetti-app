...
function StopCard({
  ...
  function handleTapCheckIn() {
    ...
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (!uid) return;
      if (stop.venueId) {
        // Known venue — upsert keyed on (user_id, venue_id, stamp_type) so we
        // never double-stamp the same venue for the same user.
      ...
      <div className="flex-1 pb-5 min-w-0 [perspective:1200px]">
        ...
        >
          ...
          <div className="[backface-visibility:hidden]">
            ...
            {/* Social buzz strip — live signals from DB */}
            <HashtagReels
