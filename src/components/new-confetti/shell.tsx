...
export function BrandMark({ size = 22, spin = false }: { size?: number; spin?: boolean }) {
  return (
    <span style={{
      ...
      fontSize: size, letterSpacing: "-0.03em", color: TOKENS.ink,
    }}>
      <span style={{
        color: TOKENS.accent1, fontSize: size * 1.05,
        display: "inline-block",
        animation: spin ? "cf-spin 3s linear infinite" : undefined,
        transformOrigin: "center",
      }}>✣</span>
      <span>confetti<span style={{ color: TOKENS.accent1 }}>.</span></span>
    </span>
  );
}
    ...
    <>
      {positions.map((p, i) => (
        <span key={i} style={{
          ...
          width: 12, height: 22, border: `2px solid ${TOKENS.ink}`,
...
export function Chip({
  children, selected, onClick, color, icon, dense = false,
}: {
  children: ReactNode;
  selected?: boolean;
  onClick?: () => void;
  ...
  return (
    <button onClick={onClick} style={{
      ...
      color: TOKENS.ink,
      fontFamily: TOKENS.ui, fontSize: dense ? 12 : 14,
...
export function ChunkyButton({
  children, onClick, disabled, variant = "primary", icon, full = true,
}: {
  children: ReactNode;
  onClick?: () => void;
  disabled?: boolean;
  ...
  const bg = variant === "primary" ? TOKENS.ink : variant === "accent" ? TOKENS.accent1 : TOKENS.paper;
...
export function Stamp({
  ...
  return (
    <span style={{
      ...
      color: TOKENS.ink,
      fontFamily: TOKENS.ui, fontSize: 11, fontWeight: 900,
...
export function Ticket({
  children, color = TOKENS.paper, notch = true, style = {},
}: {
  children: ReactNode;
  color?: string;
  notch?: boolean;
  ...
  return (
    <div style={{
      ...
      background: color, color: TOKENS.ink,
...
export function RouteDots({
  progress = 1, animate = false, size = 18,
}: {
  progress?: number;
  animate?: boolean;
  size?: number;
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        height: 3, background: TOKENS.ink,
      ...
      {[TOKENS.accent2, TOKENS.accent1, TOKENS.accent3].map((c, i) => (
        <span key={i} style={{
          ...
          border: `3px solid ${TOKENS.ink}`, borderRadius: 999, background: c,
        }} />
      ))}
    </div>
  );
