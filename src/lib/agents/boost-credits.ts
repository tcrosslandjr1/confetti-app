 ...
 *  - Business: Starter $0, Featured $199, Spotlight $499
 */
...
export type BusinessTier = "starter" | "featured" | "spotlight";
...
export interface BusinessAccount {
  ...
  inviteToken?: string; // magic-link token for initial portal access
  logoUrl?: string;
  website?: string;
  description?: string;
  city: string;
...
export const BUSINESS_TIERS: BusinessTierConfig[] = [
  {
    ...
    monthlyCredits: 100,
    features: [
      "Basic listing in Confetti discovery",
      "Claim & manage your venue page",
      "View basic check-in analytics",
  ...
  },
  {
    ...
    features: [
      ...
      "Appear in group plan suggestions",
    ],
    maxCampaigns: 3,
...
export function registerBusiness(
  ...
  teamStore.set(owner.id, owner);
  account.teamMembers.push(owner.id);
...
/** Invite a team member to a business account */
export function inviteTeamMember(
  ...
  const member: TeamMember = {
    ...
    name,
    role,
    inviteToken: `tinv_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`,
    invitedAt: new Date().toISOString(),
    isActive: false, // becomes true on accept
  };
  teamStore.set(member.id, member);
  account.teamMembers.push(member.id);
  return member;
}

/** Accept a team invite via token */
export function acceptTeamInvite(token: string): TeamMember | null {
  const member = Array.from(teamStore.values()).find((tm) => tm.inviteToken === token);
  if (!member) return null;
  member.isActive = true;
  member.acceptedAt = new Date().toISOString();
  member.inviteToken = undefined; // burn the token
  return member;
}

/** Remove a team member */
export function removeTeamMember(businessId: string, memberId: string): boolean {
  const account = businessStore.get(businessId);
  if (!account) return false;
  const member = teamStore.get(memberId);
  if (!member || member.businessId !== businessId) return false;
  if (member.role === "owner") return false; // can't remove the owner
  member.isActive = false;
  account.teamMembers = account.teamMembers.filter((id) => id !== memberId);
  return true;
}
...
export function validateBusinessInvite(token: string): BusinessAccount | null {
  return Array.from(businessStore.values()).find((b) => b.inviteToken === token) ?? null;
}
