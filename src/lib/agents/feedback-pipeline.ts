...
export type FeedbackStatus = "new" | "triaged" | "in_progress" | "shipped" | "won't_fix" | "duplicate";
...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  ui_ux: ["design", "layout", "button", "screen", "ui", "ux", "color", "font", "dark mode", "interface", "navigation", "menu"],
  performance: ["slow", "lag", "loading", "speed", "freeze", "hang", "timeout", "memory", "battery"],
  ai_recommendations: ["recommendation", "suggest", "ai", "wrong venue", "bad pick", "irrelevant", "taste", "vibe", "match"],
  group_features: ["group", "party", "friends", "invite", "vote", "merge", "collab", "room"],
  booking_reservations: ["booking", "reservation", "table", "rsvp", "cancel", "waitlist", "confirm"],
  payments_billing: ["payment", "billing", "charge", "refund", "subscription", "price", "coupon", "credit"],
  ...
  onboarding: ["onboarding", "signup", "sign up", "new user", "tutorial", "getting started", "first time"],
  social_sharing: ["share", "social", "post", "story", "instagram", "tiktok", "twitter"],
};

const SENTIMENT_KEYWORDS = {
  positive: ["love", "great", "awesome", "amazing", "perfect", "excellent", "best", "wonderful", "fantastic", "brilliant", "helpful", "beautiful", "intuitive"],
  negative: ["hate", "terrible", "awful", "worst", "horrible", "broken", "useless", "annoying", "frustrating", "disappointed", "waste", "garbage", "trash"],
};

const PRIORITY_KEYWORDS: Record<FeedbackPriority, string[]> = {
  critical: ["crash", "data loss", "can't login", "security", "locked out", "money", "charged", "urgent", "broken completely"],
...
const STOP_WORDS = new Set([
  ...
  "what", "which", "who", "whom", "app", "confetti",
]);
...
export function updateFeedbackStatus(feedbackId: string, status: FeedbackStatus): FeedbackItem | null {
  const item = feedbackStore.get(feedbackId);
  if (!item) return null;
...
export function getFeedbackQueue(filter?: FeedbackQueueFilter): FeedbackItem[] {
  ...
  const priorityRank: Record<FeedbackPriority, number> = { critical: 4, high: 3, medium: 2, low: 1 };
...
export function getFeedbackTrends(days: number = 30): FeedbackTrend[] {
  ...
  const categoryData = new Map<string, { count: number; sentimentSum: number; recentHalf: number; olderHalf: number }>();

  for (const item of recentItems) {
    const cat = item.category;
    const data = categoryData.get(cat) ?? { count: 0, sentimentSum: 0, recentHalf: 0, olderHalf: 0 };
    data.count++;
    data.sentimentSum += item.aiSentiment === "positive" ? 1 : item.aiSentiment === "negative" ? -1 : 0;
...
export function getFeedbackStats(): FeedbackStats {
  ...
  const byType: Record<FeedbackType, number> = {
    bug_report: 0, feature_request: 0, complaint: 0, praise: 0, suggestion: 0,
  };
  const byStatus: Record<FeedbackStatus, number> = {
    new: 0, triaged: 0, in_progress: 0, shipped: 0, "won't_fix": 0, duplicate: 0,
  };
  const bySource: Record<FeedbackSource, number> = {
    in_app: 0, app_store: 0, email: 0, social: 0, support_ticket: 0,
  };
...
export function seedFeedbackDemo(): FeedbackItem[] {
  const samples: Array<{
  ...
  }> = [
    {
      ...
      description: "Every time I switch between the group chat and the group plan tab, the app crashes. Happens on iOS 18. This is really frustrating because I lose my place in the conversation.",
      source: "in_app",
      userId: "user_001",
    },
    {
      ...
      description: "Love the app but using it at night in bars and clubs is way too bright. A dark mode option would be amazing. Maybe even an auto-switch based on time of day?",
      source: "in_app",
      userId: "user_002",
    },
    {
      ...
      description: "It would be great to have dark mode. The screen is very bright when I'm out at restaurants in the evening. A darker theme would help a lot.",
      source: "app_store",
      userId: "user_009",
    },
    {
      ...
      description: "This app is amazing! Planned a whole birthday night out for my friend and every single recommendation was perfect. The AI really understood the vibe we were going for. Love it!",
      source: "app_store",
      userId: "user_003",
    },
    {
      ...
      description: "I live in a smaller city and the AI keeps recommending venues that are 30+ miles away. The recommendations feel irrelevant and the app is basically useless outside of major cities.",
      source: "email",
      userId: "user_004",
    },
    {
      ...
      description: "It would be nice if the itinerary stops could open directly in Apple Maps or Google Maps for turn-by-turn navigation between venues.",
      source: "in_app",
      userId: "user_005",
    },
    {
      ...
      description: "After updating the app I can't login anymore. It just shows a white screen. I've tried reinstalling and resetting my password but nothing works. Locked out of my account completely.",
    ...
    },
    {
      ...
      description: "When planning with a group, it would be helpful to have a shared budget tracker so everyone can see the estimated cost per person for the night out.",
      source: "in_app",
      userId: "user_007",
    },
    {
      ...
      description: "I'm getting way too many push notifications from this app. Multiple per day about venues I'm not even interested in. There's no way to control notification frequency in settings.",
      source: "app_store",
      userId: "user_008",
    },
    {
      ...
      description: "Would be cool if each venue had a Spotify playlist that matches the vibe so you can preview what the atmosphere will be like before you go.",
      source: "social",
      userId: "user_010",
    },
  ];
