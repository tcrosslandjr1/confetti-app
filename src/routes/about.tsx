...
const VALUES = [
  { icon: Heart, title: "Show up.", body: "The best memories happen IRL. Our job is to remove every reason to bail.", color: "bg-coral" },
  { icon: Compass, title: "Off the same five.", body: "We surface the patio, the paint night, the dive — not the same chain restaurant.", color: "bg-purple" },
  { icon: Users, title: "Match the crew.", body: "Different ages, different energies. The plan bends to the people.", color: "bg-gold" },
  { icon: Coffee, title: "Built in the open.", body: "Small team, loud opinions, fast iterations. Tell us what to plan next.", color: "bg-mint" },
];

const TEAM = [
  { name: "Mara Lin", role: "Co-founder · Product", bio: "Ex-design lead. Believes a good Friday saves a bad week.", initials: "ML", color: "bg-coral", rot: "-rotate-2" },
  { name: "Devin Ortiz", role: "Co-founder · Engineering", bio: "Built routing for two delivery apps. Now routes humans to dinner.", initials: "DO", color: "bg-purple", rot: "rotate-1" },
  { name: "Priya Shah", role: "Head of Taste", bio: "Restaurant critic turned curator. Knows where the regulars actually sit.", initials: "PS", color: "bg-gold", rot: "-rotate-1" },
  { name: "Theo Walker", role: "Engineering", bio: "Maps nerd. Has opinions about transfers, transit, and the perfect 9-min walk.", initials: "TW", color: "bg-mint", rot: "rotate-2" },
];
      ...
      <section className="relative overflow-hidden border-b-2 border-ink">
        ...
        <div className="mx-auto max-w-7xl px-4 pb-20 pt-16 sm:px-6 lg:px-8 lg:pb-28 lg:pt-20">
          ...
          <p className="mt-8 max-w-2xl text-lg leading-snug">
            Loop started with one frustration: it's harder to decide what to do than to actually do it.
      ...
      <section className="border-b-2 border-ink">
        <div className="mx-auto grid max-w-7xl gap-12 px-4 py-20 sm:px-6 lg:grid-cols-12 lg:px-8">
          <Reveal className="lg:col-span-5">
            <span className="font-mono text-xs uppercase tracking-[0.25em] text-ink/60">/ the story</span>
          ...
          <Reveal className="space-y-5 text-lg leading-relaxed lg:col-span-7" delay={120}>
            ...
            <p>
              So we built a planner that does the messy part — picking the spots, timing the day, sorting the route, holding the reservations — and leaves you with the fun part: <span className="font-serif italic">showing up.</span>
            </p>
            <p>
      ...
      <section className="border-b-2 border-ink bg-cream/60">
        <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="mb-10 flex items-end justify-between gap-6">
            ...
            <span className="hidden font-mono text-xs uppercase tracking-widest text-ink/60 sm:inline">/ four rules</span>
          </div>
              ...
              return (
                <Reveal key={v.title} delay={i * 60}>
                  <div className={`h-full rounded-2xl border-2 border-ink bg-cream p-6 shadow-brut transition-pop hover:-translate-x-1 hover:-translate-y-1 hover:shadow-brut-lg ${rot}`}>
                    ...
                    <h3 className="mt-5 font-display text-2xl font-extrabold leading-tight">{v.title}</h3>
      ...
      <section className="border-b-2 border-ink">
        <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="mb-10 flex items-end justify-between gap-6">
            ...
            <span className="hidden font-mono text-xs uppercase tracking-widest text-ink/60 sm:inline">/ small + loud</span>
          </div>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {TEAM.map((t, i) => (
              <Reveal key={t.name} delay={i * 60}>
                <div className={`group h-full rounded-2xl border-2 border-ink bg-cream p-6 shadow-brut transition-pop hover:-translate-x-1 hover:-translate-y-1 hover:shadow-brut-lg ${t.rot} hover:rotate-0`}>
                  ...
                  <h3 className="mt-5 font-display text-xl font-extrabold leading-tight">{t.name}</h3>
                  <p className="mt-1 font-mono text-[11px] uppercase tracking-widest text-ink/60">{t.role}</p>
      ...
      <section className="relative overflow-hidden border-b-2 border-ink bg-ink text-cream">
        ...
        <div className="mx-auto max-w-4xl px-4 py-24 text-center sm:px-6">
          <h2 className="font-display text-5xl font-extrabold leading-[0.95] tracking-tight sm:text-6xl">
            Got a vibe we should plan for?<br />
            <span className="font-serif italic font-normal text-gold">Tell us.</span>
          </h2>
          <p className="mx-auto mt-5 max-w-xl text-lg text-cream/80">We're building this in the open and we read everything.</p>
          <Link
            to="/contact"
