...
function NotificationRoutingCell({ venueId, cache, resolve, }: {
    venueId: string | null;
    cache: Map<string, Routing | null>;
    resolve: (id: string) => Promise<Routing | null>;
}) {
    const [routing, setRouting] = useState<Routing | null | undefined>(venueId ? cache.get(venueId) : null);
    useEffect(() => {
        let cancelled = false;
        if (!venueId) {
            setRouting(null);
    ...
    return (<div className="space-y-1">
      ...
      <span className={`inline-flex items-center rounded-full border px-1.5 py-0.5 text-[10px] ${SOURCE_BADGE[routing.source] ?? "border-border bg-muted text-muted-foreground"}`}>
...
function AdminBookingsPage() {
    ...
    const resolveRouting = useMemo(() => (venueId: string) => {
    ...
    const filtered = useMemo(() => {
        return bookings.filter((b) => {
            ...
            if (tab !== "all" && status !== tab)
                return false;
            if (venueFilter !== "all" && b.venue_name !== venueFilter)
                return false;
            if (query) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-card p-3 shadow-card">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search guest, venue, or booking ID…" className="pl-9"/>
        </div>
        <div className="flex items-center gap-2">
          <Filter className="h-4 w-4 text-muted-foreground"/>
          <select value={venueFilter} onChange={(e) => setVenueFilter(e.target.value)} className="rounded-md border border-border bg-background px-3 py-2 text-sm">
          ...
          <TableBody>
            ...
            return (<TableRow key={b.id}>
                    ...
                    return (<div className="space-y-1 text-xs">
                            {drinks.length > 0 && (<div className="flex items-start gap-1">
                                <Wine className="mt-0.5 h-3 w-3 shrink-0 text-muted-foreground"/>
                                <span>
                                  {drinks
                                .map((d) => `${d.qty}× ${d.name}${d.notes ? ` (${d.notes})` : ""}`)
                                .join(", ")}
                                </span>
                              </div>)}
                            {b.seating_preference && (<div className="flex items-start gap-1">
                    ...
                    <TableCell>
                      <NotificationRoutingCell venueId={b.venue_id} cache={routingCacheRef.current} resolve={resolveRouting}/>
                    </TableCell>
                    <TableCell>
