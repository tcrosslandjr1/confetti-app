...
import { adminListVenueClaims, adminApproveVenueClaim, adminRejectVenueClaim } from "@/lib/business-onboarding.functions";
...
export const Route = createLazyFileRoute("/admin/business-claims")({
...
function AdminBusinessClaimsPage() {
    ...
    const approveMut = useMutation({
        mutationFn: (claimId: string) => approve({ data: { claimId, adminNote: notes[claimId] || undefined } }),
        onSuccess: () => {
    ...
    return (<div className="space-y-6 p-6">
      ...
      <div className="flex gap-2">
        {(["pending", "approved", "rejected", "all"] as Status[]).map((s) => (<Button key={s} size="sm" variant={status === s ? "default" : "outline"} onClick={() => setStatus(s)}>
      ...
      {!isLoading && claims.length === 0 && (<p className="text-sm text-muted-foreground">No claims in this view.</p>)}

      <div className="space-y-4">
        {claims.map((c) => {
            const venueName = c.venue?.name ?? c.proposed_name ?? "Unlisted venue";
            const isPending = c.status === "pending";
            return (<div key={c.id} className="rounded-xl border border-border bg-card p-5 shadow-sm space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-lg font-medium text-foreground">{venueName}</h3>
                    <Badge variant={c.status === "approved"
                    ? "default"
                    : c.status === "rejected"
                        ? "destructive"
                        : "secondary"}>
              ...
              <div className="rounded-lg bg-muted/40 p-3 text-sm space-y-2">
                ...
                {c.evidence_handle && (<p className="text-muted-foreground">Handle: @{c.evidence_handle}</p>)}
                {c.evidence_url && (<a href={c.evidence_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary hover:underline">
                    Evidence link <ExternalLink className="h-3 w-3"/>
                  </a>)}
                {c.evidence_email && (<p className="text-muted-foreground">Email: {c.evidence_email}</p>)}
                {c.notes && <p className="text-muted-foreground italic">"{c.notes}"</p>}
                {c.admin_note && c.status !== "pending" && (<p className="text-foreground">
                    <span className="font-medium">Admin note:</span> {c.admin_note}
                  </p>)}
              </div>

              {isPending && (<div className="space-y-2">
                  <Textarea placeholder="Admin note (required to reject)" value={notes[c.id] ?? ""} onChange={(e) => setNotes((n) => ({ ...n, [c.id]: e.target.value }))} rows={2}/>
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => approveMut.mutate(c.id)} disabled={approveMut.isPending}>
                      <CheckCircle2 className="mr-1 h-4 w-4"/> Approve
                    </Button>
                    <Button size="sm" variant="destructive" onClick={() => rejectMut.mutate(c.id)} disabled={rejectMut.isPending}>
