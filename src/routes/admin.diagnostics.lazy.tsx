...
import { Activity, CheckCircle2, XCircle, AlertTriangle, RefreshCw, ChevronLeft, Database, Globe, KeyRound, Server, Cpu } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { VITE_GOOGLE_MAPS_API_KEY } from "@/lib/config";

export const Route = createLazyFileRoute("/admin/diagnostics")({
...
function Row({ check }: {
    ...
    return (<div className="flex items-start justify-between gap-4 border-b border-border/60 py-3 last:border-0">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          {check.detail && (<div className="mt-0.5 text-xs text-muted-foreground break-all">{check.detail}</div>)}
        </div>
      </div>
    </div>);
}
...
function useAuthCheck() {
    return useQuery({
        ...
        queryFn: async (): Promise<Check> => {
            ...
            if (error)
                return { label: "Auth service", status: "fail", detail: error.message };
            return {
                label: "Auth service",
                status: "ok",
                detail: data.session
...
function AdminDiagnostics() {
    ...
    const buildChecks: Check[] = [
        ...
        },
        {
            ...
            detail: typeof navigator !== "undefined" && navigator.onLine
    ...
    const summary = [...buildChecks, ...envChecks, ...backendChecks].reduce((acc, c) => {
    ...
    return (<div className="min-h-screen bg-background">
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mb-6 flex flex-wrap gap-2 text-xs">
          ...
          {summary.pending ? (<span className="rounded-full bg-muted px-3 py-1 font-medium text-muted-foreground">
        ...
        <div className="space-y-4">
          <Section icon={Cpu} title="Build & preview">
            {buildChecks.map((c) => (<Row key={c.label} check={c}/>))}
          </Section>

          <Section icon={KeyRound} title="Environment variables">
            {envChecks.map((c) => (<Row key={c.label} check={c}/>))}
          </Section>

          <Section icon={Database} title="Backend connectivity">
            {backendChecks.map((c) => (<Row key={c.label} check={c}/>))}
          </Section>
