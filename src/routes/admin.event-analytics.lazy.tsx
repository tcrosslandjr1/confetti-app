...
export const Route = createLazyFileRoute("/admin/event-analytics")({
...
function EventAnalyticsPage() {
    ...
    return (<div className="space-y-6 p-4 sm:p-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex rounded-full border border-border bg-card p-0.5">
            {WINDOWS.map((w) => (<button key={w.id} onClick={() => setWindowId(w.id)} className={`rounded-full px-3 py-1 text-xs font-semibold ${windowId === w.id ? "bg-ink text-cream" : "text-muted-foreground hover:text-ink"}`}>
                {w.label}
              </button>))}
          </div>
          <button onClick={() => void load()} className="inline-flex h-8 items-center gap-1.5 rounded-full border border-border bg-card px-3 text-xs font-semibold hover:border-ink">
            <RefreshCw className={`h-3 w-3 ${loading ? "animate-spin" : ""}`}/> Refresh
          </button>
        </div>
      </header>

      {err && (<div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">
          {err}
        </div>)}
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Kpi icon={Timer} label="Median time-to-interaction" value={stats.medianTti != null ? `${stats.medianTti} ms` : "—"}/>
        <Kpi icon={AlertTriangle} label="Errors" value={stats.errors} tone={stats.errors > 0 ? "warn" : "ok"}/>
      </div>

      {/* Per-path table */}
      <Section title="By path" icon={BarChart3}>
        <Table headers={["Path", "Views", "CTA clicks", "CTR", "Median TTI", "Errors"]} rows={stats.byPath.map((p) => [
            p.path,
            String(p.views),
            String(p.ctaClicks),
      ...
      <Section title="Top CTAs" icon={MousePointerClick}>
        <Table headers={["CTA name", "Path", "Clicks"]} rows={stats.topCtas.map((c) => [c.name, c.path, String(c.count)])}/>
      </Section>

      {/* Scroll depth */}
      <Section title="Scroll depth (% of sessions reaching milestone)" icon={BarChart3}>
        <Table headers={["Path", "≥25%", "≥50%", "≥75%", "100%"]} rows={stats.scrollByPath.map((s) => [
            s.path,
      ...
      <Section title="Recent errors" icon={AlertTriangle}>
        {stats.recentErrors.length === 0 ? (<p className="text-sm text-muted-foreground">No errors in window.</p>) : (<ul className="space-y-2">
            {stats.recentErrors.slice(0, 20).map((e) => (<li key={e.id} className="rounded-lg border border-border bg-card p-3 text-xs">
                ...
                {e.metadata && (<pre className="mt-1 overflow-x-auto rounded bg-muted/40 p-2 text-[11px]">
...
function Kpi({ icon: Icon, label, value, tone, }: {
    ...
    return (<div className={`rounded-xl border bg-card p-4 ${tone === "warn" ? "border-amber-400/60" : "border-border"}`}>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border text-left text-[11px] uppercase tracking-wider text-muted-foreground">
            {headers.map((h) => (<th key={h} className="py-2 pr-3 font-semibold">
                {h}
              </th>))}
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (<tr key={i} className="border-b border-border/40 last:border-0">
              {r.map((c, j) => (<td key={j} className="py-2 pr-3 font-mono text-xs">
                  {c}
                </td>))}
            </tr>))}
        </tbody>
...
function computeStats(rows: Row[]) {
    ...
    for (const r of rows) {
        const p = r.path;
        if (!pathMap.has(p))
            pathMap.set(p, { views: 0, ctaClicks: 0, tti: [], errors: 0 });
        const s = pathMap.get(p)!;
        if (r.event_type === "pageview")
            s.views++;
        else if (r.event_type === "cta_click")
            s.ctaClicks++;
        else if (r.event_type === "time_to_interaction" && r.value != null)
            s.tti.push(Number(r.value));
    ...
    for (const r of rows) {
        if (r.event_type !== "cta_click")
            continue;
        const key = `${r.event_name}::${r.path}`;
        if (!ctaMap.has(key))
            ctaMap.set(key, { name: r.event_name, path: r.path, count: 0 });
    ...
    for (const r of rows) {
        ...
        if (r.event_type === "scroll_depth" && r.value != null) {
            ...
            if (v >= 25)
                s.p25.add(r.session_id);
            if (v >= 50)
                s.p50.add(r.session_id);
            if (v >= 75)
                s.p75.add(r.session_id);
            if (v >= 100)
                s.p100.add(r.session_id);
        }
    }
