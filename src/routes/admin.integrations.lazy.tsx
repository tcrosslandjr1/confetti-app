...
export const Route = createLazyFileRoute("/admin/integrations")({
...
function AdminIntegrationsPage() {
    ...
    const runTest = async (i: Integration) => {
        ...
        try {
            ...
            if (r.ok)
                toast.success(`${i.name} OK`, { description: r.detail });
            else
                toast.error(`${i.name} failed`, { description: r.detail });
        }
        catch (e: any) {
            setStatuses((s) => ({
                ...s,
    ...
    return (<div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1">
          <Button onClick={runAllDiagnostics} disabled={runningAll}>
            {runningAll ? (<Loader2 className="mr-1 h-4 w-4 animate-spin"/>) : (<RefreshCw className="mr-1 h-4 w-4"/>)}
            Run diagnostics
          </Button>
          {lastRun && (<span className="text-[11px] text-muted-foreground">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {INTEGRATIONS.map((i) => {
            ...
            const r = st.result;
            return (<article key={i.key} className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-5 shadow-card">
              ...
              <div className="flex flex-wrap items-center gap-2 rounded-xl bg-muted/60 px-3 py-2 text-xs">
                ...
                {r.keyMasked && (<span className="font-mono text-muted-foreground">· {r.keyMasked}</span>)}
                <span className="text-muted-foreground">· managed in Lovable Cloud → Secrets</span>
              </div>

              {r.checks && r.checks.length > 0 && (<ul className="space-y-1 text-xs">
                  {r.checks.map((c, idx) => (<li key={idx} className="flex items-start gap-2">
                      {c.ok ? (<CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-600"/>) : (<XCircle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-destructive"/>)}
                      <span>
              ...
              {!r.ok && r.detail && !r.checks && (<p className="rounded-lg bg-destructive/10 px-3 py-2 text-xs text-destructive">
                  {r.detail}
                </p>)}

              {r.remediation && (<p className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-900 dark:text-amber-200">
...
function ViralRefreshPanel() {
    ...
    return (<section className="space-y-3 rounded-2xl border border-border bg-card p-5 shadow-card">
      <div>
      ...
      {runs.length > 0 && (<div className="space-y-1.5 pt-2">
          <p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
            Recent runs
          </p>
          <ul className="divide-y divide-border rounded-xl border border-border bg-background text-xs">
            {runs.map((r) => (<li key={r.id} className="flex items-center justify-between gap-2 px-3 py-2">
