...
function AdminLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(60%_50%_at_20%_0%,hsl(var(--primary)/0.12),transparent_70%),radial-gradient(50%_40%_at_100%_20%,hsl(var(--accent)/0.10),transparent_70%)]"
      />

      <div className="mx-auto flex min-h-screen max-w-md flex-col px-6 py-10">
        <Link
          to="/"
          className="inline-flex items-center gap-1 text-xs font-mono uppercase tracking-widest text-muted-foreground transition-colors hover:text-foreground"
        >
        ...
        <form onSubmit={onSubmit} className="mt-8 space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-border bg-card py-4 pl-11 pr-4 text-sm shadow-card outline-none ring-primary/40 transition focus:ring-2"
            />
          </div>
          <div className="relative">
          ...
          <button
            disabled={busy}
            className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-primary py-4 text-sm font-semibold text-primary-foreground shadow-pop transition-pop hover:brightness-105 active:scale-95 disabled:opacity-60"
          >
