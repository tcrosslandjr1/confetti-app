...
import { AlertTriangle, CheckCircle2, Cpu, RefreshCw, ScrollText, ShieldAlert, UploadCloud, XCircle } from "lucide-react";
...
export const Route = createLazyFileRoute("/admin/logs")({
...
function AdminLogsPage() {
    ...
    return (<div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <button onClick={() => window.location.reload()} className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-semibold hover:border-ink">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border pb-2">
        {TABS.map((t) => {
            ...
            return (<button key={t.id} onClick={() => setTab(t.id)} className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold transition ${active
...
/*  Panels                                                             */
/* ------------------------------------------------------------------ */
function ApiErrorsPanel({ filters }: {
    ...
    return (<LogTable loading={isLoading} empty="No API/delivery errors match these filters." columns={["When", "Status", "Subject", "Recipient", "Source", "Error"]} rows={filtered.map((r) => [
...
function AiJobsPanel({ filters }: {
    ...
    return (<LogTable loading={isLoading} empty="No failed AI jobs match these filters." columns={["When", "Kind", "Target", "Status", "Error"]} rows={filtered.map((r) => [
            timeAgo(r.created_at),
...
function UploadsPanel({ filters }: {
    ...
    return (<LogTable loading={isLoading} empty="No failed uploads match these filters." columns={["When", "Venue", "Kind", "Source", "Status"]} rows={filtered.map((r) => [
            timeAgo(r.created_at),
...
function AdminActionsPanel({ filters }: {
    filters: LogFilterState;
}) {
    ...
    return (<LogTable loading={false} empty="No admin actions match these filters." columns={["When", "Admin", "Action", "Entity", "Target", "Summary"]} rows={filtered.map((e) => [
            timeAgo(e.at),
            e.admin,
...
function SecurityPanel({ filters }: {
    filters: LogFilterState;
}) {
    const { data, isLoading } = useQuery({
    ...
    return (<LogTable loading={isLoading} empty={<span className="inline-flex items-center gap-2">
          <CheckCircle2 className="h-4 w-4 text-emerald-600"/> No security events match these
          filters.
        </span>} columns={["When", "Kind", "Title", "Detail"]} rows={filtered.map((r) => [
            timeAgo(r.created_at),
            <span key="k" className="font-mono text-[11px] uppercase tracking-wider">
          {r.kind}
        </span>,
...
/*  Primitives                                                         */
/* ------------------------------------------------------------------ */
function LogTable({ loading, empty, columns, rows, }: {
    loading: boolean;
    empty: ReactNode;
    columns: string[];
    rows: ReactNode[][];
    ...
    if (loading) {
        return (<div className="grid place-items-center rounded-2xl border border-dashed border-border p-12 text-sm text-muted-foreground">
        Loading…
      </div>);
    }
    if (rows.length === 0) {
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-left text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
            <tr>
              {columns.map((c) => (<th key={c} className="px-3 py-2 font-semibold">
                  {c}
                </th>))}
            </tr>
          </thead>
          <tbody>
            {rows.map((cells, i) => (<tr key={i} className="border-t border-border/60 hover:bg-muted/30">
                {cells.map((cell, j) => (<td key={j} className="px-3 py-2 align-middle">
                    {cell}
                  </td>))}
              </tr>))}
          </tbody>
...
function StatusPill({ tone, children }: {
    ...
    return (<span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 font-mono text-[10px] font-bold uppercase tracking-wider ${cls}`}>
