...
function AdminMarqueePage() {
  ...
  async function load() {
    ...
    else setRows((data ?? []) as Sponsorship[]);
    setLoading(false);
  }
  ...
  async function remove(row: Sponsorship) {
    if (!confirm(`Delete ${row.brand} · ${row.occasion}?`)) return;
        ...
        <aside className="space-y-3 rounded-xl border border-border bg-card p-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="rooftop o'clock → sunset + spritz"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="/wizard?occasion=rooftop"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1.5">
              ...
              <Input
                id="runs_from"
                type="datetime-local"
                value={toLocalInput(form.runs_from)}
                onChange={(e) => setForm({ ...form, runs_from: fromLocalInput(e.target.value) })}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                id="runs_until"
                type="datetime-local"
                value={toLocalInput(form.runs_until)}
                onChange={(e) => setForm({ ...form, runs_until: fromLocalInput(e.target.value) })}
              />
            </div>
          </div>
