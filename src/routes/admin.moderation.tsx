...
import {
  adminListEventsForModeration,
  adminDecideEvent,
  adminListVenuesForModeration,
  adminDecideVenue,
...
function EventsModerationTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <ModerationDecideDialog
        open={!!decision}
        onOpenChange={(o) => !o && setDecision(null)}
        decision={decision?.kind ?? null}
        targetLabel={decision ? `event "${decision.label}"` : ""}
...
function EventStatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge className="bg-gold/20 text-foreground">
      <AlertTriangle className="mr-1 h-3 w-3" /> Pending
    </Badge>
  );
}
...
const VENUE_TABS: { key: VenueStatus; label: string }[] = [
  ...
  return (
    <div className="space-y-4">
      ...
      <ModerationDecideDialog
        ...
        onOpenChange={(o) => !o && setDecision(null)}
        decision={decision?.kind ?? null}
        targetLabel={decision ? `venue "${decision.label}"` : ""}
        pending={decideMut.isPending}
        onConfirm={(note) =>
          decision &&
          decideMut.mutate({
...
function VenueStatusBadge({ verified }: { verified: boolean | null }) {
  if (verified)
    return (
      <Badge className="bg-emerald-500/15 text-emerald-700">
        <CheckCircle2 className="mr-1 h-3 w-3" /> Approved
...
function ModerationToolbar<T extends string>({
  tabs,
  active,
  onTab,
  query,
...
function ReportsTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <EmptyState label="Nothing to moderate here. Inbox zero ✨" />
      ) : (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() => decideReport(r.id, "approved")}
                  >
                    <CheckCircle2 className="mr-1 h-3.5 w-3.5" /> Approve
                  </Button>
                  <Button
                    ...
                    onClick={() => decideReport(r.id, "removed")}
                    className="text-destructive hover:text-destructive"
                  >
