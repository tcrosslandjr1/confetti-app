...
function AdminNotificationsPage() {
    ...
    const load = async () => {
        ...
        const { data, error } = await supabase
            ...
            .limit(200);
        if (error)
            toast.error("Couldn't load deliveries", { description: error.message });
        else
    ...
    useEffect(() => {
        ...
        const ch = supabase
            .channel("admin-bnd")
            .on("postgres_changes", { event: "*", schema: "public", table: "booking_notification_deliveries" }, () => void load())
            .subscribe();
        return () => {
    ...
    const filtered = useMemo(() => {
        ...
        return rows.filter((r) => {
            ...
            if (!q)
                return true;
            return ((r.venue_name ?? "").toLowerCase().includes(q) ||
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        ...
        ].map((s) => (<div key={s.k} className="rounded-2xl border border-border bg-card p-4 shadow-card">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex flex-wrap gap-1">
          {STATUSES.map((s) => (<button key={s} onClick={() => setStatus(s)} className={`rounded-full border px-3 py-1 text-xs capitalize transition ${status === s
        ...
        <div className="relative ml-auto min-w-[260px] flex-1 sm:flex-none">
          ...
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by venue, recipient, subject…" className="pl-9"/>
        </div>
      </div>
      ...
      {loading ? (<div className="grid place-items-center rounded-2xl border border-dashed border-border bg-card/50 p-10 text-sm text-muted-foreground">
        ...
        </div>) : (<div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((r) => {
                ...
                return (<tr key={r.id} className="border-t border-border/60">
                    <td className="px-4 py-2 align-top text-xs text-muted-foreground">
                      {timeAgo(r.created_at)}
                      {r.test && (<span className="ml-1 rounded bg-muted px-1 py-0.5 font-mono text-[10px] uppercase">
                          test
                        </span>)}
                    </td>
                    ...
                    <td className="px-4 py-2 align-top">
                      ...
                      {r.error && (<p className="mt-1 max-w-[28ch] truncate text-[11px] text-destructive" title={r.error}>
                          {r.error}
                        </p>)}
                    </td>
                    <td className="px-4 py-2 text-right align-top">
                      <div className="flex justify-end gap-1">
                        {canResend && (<Button size="sm" variant="outline" disabled={resendingId === r.id} onClick={() => void onResend(r)}>
                            {resendingId === r.id ? (<Loader2 className="mr-1 h-3.5 w-3.5 animate-spin"/>) : (<Send className="mr-1 h-3.5 w-3.5"/>)}
                            Resend
                          </Button>)}
        ...
        <DialogContent className="max-w-lg">
          {preview && (<>
              ...
              <div className="space-y-3 text-sm">
                ...
                {preview.error && (<p className="rounded-md border border-destructive/30 bg-destructive/10 p-2 text-xs text-destructive">
                    {preview.error}
                  </p>)}
              </div>
            </>)}
