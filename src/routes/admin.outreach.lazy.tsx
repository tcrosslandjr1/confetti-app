...
import { getOutreachRanking, getOutreachCsv, getLatestOutreachSnapshot, type LatestOutreachSnapshot, type OutreachVenue } from "@/lib/outreach-ranking.functions";

export const Route = createLazyFileRoute("/admin/outreach")({
...
function AdminOutreachPage() {
    ...
    return (<div className="p-6 space-y-6 max-w-6xl">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Select value={String(days)} onValueChange={(v) => {
            const d = Number(v);
            setDays(d);
            load(d);
        }}>
      ...
      {generatedAt && (<div className="text-xs text-muted-foreground">
      ...
      {loading && venues.length === 0 ? (<div className="flex items-center justify-center py-16 text-muted-foreground">
          <Loader2 className="h-6 w-6 animate-spin"/>
        </div>) : venues.length === 0 ? (<div className="rounded-lg border border-dashed p-10 text-center text-sm text-muted-foreground">
          ...
          <section>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {top.map((v, i) => (<article key={v.venue_id} className="rounded-lg border bg-card p-4 flex flex-col gap-2">
                  ...
                  {v.staff_email ? (<div className="text-xs text-muted-foreground flex items-center gap-1">
          ...
          {rest.length > 0 && (<section>
              ...
              <div className="rounded-lg border overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {rest.map((v, i) => (<tr key={v.venue_id} className="border-t">
