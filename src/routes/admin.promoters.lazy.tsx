...
import { adminListPendingSubmissions, adminVerifySubmission, adminListPromoters, adminSetPromoterStatus } from "@/lib/promoter.functions";

export const Route = createLazyFileRoute("/admin/promoters")({
    ...
    return (<div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      ...
      <section className="space-y-3">
        ...
        {subs.map((s: any) => {
            ...
            return (<Card key={s.id} className="p-5 space-y-3">
              ...
              <a href={s.content_url} target="_blank" rel="noreferrer" className="text-primary hover:underline text-sm break-all">
                {s.content_url}
              </a>
      ...
      <section className="space-y-3">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {promoters.map((p: any) => (<Card key={p.id} className="p-4">
              ...
              <div className="flex gap-2 mt-3">
                {p.status !== "approved" && (<Button size="sm" onClick={() => statusMut.mutate({ promoter_id: p.id, status: "approved" })}>
                    Approve
                  </Button>)}
                {p.status !== "rejected" && (<Button size="sm" variant="outline" onClick={() => statusMut.mutate({ promoter_id: p.id, status: "rejected" })}>
                    Reject
                  </Button>)}
                {p.status !== "suspended" && (<Button size="sm" variant="ghost" onClick={() => statusMut.mutate({ promoter_id: p.id, status: "suspended" })}>
                    Suspend
                  </Button>)}
              </div>
            </Card>))}
...
function VerifyControls({ onDecide, pending, }: {
    ...
    return (<div className="space-y-2">
      ...
      <div className="flex gap-2 justify-end">
        <Button size="sm" variant="outline" disabled={pending} onClick={() => onDecide("needs_revision", notes)}>
          Needs revision
        </Button>
        <Button size="sm" variant="destructive" disabled={pending} onClick={() => onDecide("rejected", notes)}>
          Reject
        </Button>
