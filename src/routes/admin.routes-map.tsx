...
function toCSV(rows: RouteEntry[]): string {
  ...
  const body = rows.map((r) =>
    [r.path, r.purpose, r.oneJob, (r.flags ?? []).join("|"), r.notes ?? ""].map(esc).join(","),
  );
...
function RoutesMapPage() {
  ...
  const purposes = useMemo(
    () => Array.from(new Set(ROUTE_REGISTRY.map((r) => r.purpose))) as RoutePurpose[],
    [],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return ROUTE_REGISTRY.filter((r) => {
      ...
      if (flag === "any" && !(r.flags && r.flags.length)) return false;
      if (flag === "none" && r.flags && r.flags.length) return false;
      if (flag !== "all" && flag !== "any" && flag !== "none") {
        if (!r.flags?.includes(flag)) return false;
      }
  ...
  const byPurpose = useMemo(() => {
    ...
    for (const r of ROUTE_REGISTRY) map.set(r.purpose, (map.get(r.purpose) ?? 0) + 1);
    return Array.from(map.entries()).sort((a, b) => b[1] - a[1]);
  }, []);

  const flaggedCount = useMemo(
    () => ROUTE_REGISTRY.filter((r) => r.flags && r.flags.length).length,
    [],
  );
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="rounded-lg border p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {byPurpose.map(([p, n]) => (
            <button
              key={p}
              onClick={() => setPurpose(purpose === p ? "all" : p)}
              className={`rounded-full px-3 py-1 text-xs font-medium transition ${PURPOSE_TONE[p]} ${
                purpose === p ? "ring-2 ring-offset-1" : "opacity-80 hover:opacity-100"
              }`}
            >
              {PURPOSE_LABELS[p]} · {n}
            </button>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Input
          placeholder="Search path, job, notes…"
          value={q}
        ...
        <select
          value={purpose}
          onChange={(e) => setPurpose(e.target.value as RoutePurpose | "all")}
        ...
        >
          ...
          {(Object.keys(FLAG_LABELS) as ConsolidationFlag[]).map((f) => (
            <option key={f} value={f}>
              {FLAG_LABELS[f]}
            </option>
          ))}
        </select>
        {(q || purpose !== "all" || flag !== "all") && (
          <Button
            variant="ghost"
            size="sm"
            onClick={() => {
          ...
          <TableBody>
            {filtered.map((r) => (
              <TableRow key={r.path}>
                ...
                <TableCell>
                  <span
                    className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${PURPOSE_TONE[r.purpose]}`}
                  >
