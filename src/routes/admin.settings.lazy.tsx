...
import { CheckCircle2, ExternalLink, KeyRound, Loader2, Lock, RefreshCw, Settings, ShieldCheck, XCircle } from "lucide-react";
      ...
      <article className="rounded-3xl border border-border bg-card p-6 shadow-card space-y-5">
        ...
        <div className="rounded-2xl bg-muted/60 p-4 space-y-2 text-sm">
          <div className="flex items-center gap-2">
            ...
            {diag?.keyMasked && (<span className="font-mono text-xs text-muted-foreground">· {diag.keyMasked}</span>)}
          </div>
        ...
        <ol className="space-y-2 text-sm text-muted-foreground list-decimal list-inside">
          <li>
            In{" "}
            <a href="https://console.cloud.google.com/apis/library/places-backend.googleapis.com" target="_blank" rel="noreferrer" className="text-primary underline">
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1">
          <Button onClick={() => {
            ...
            <a href="https://docs.lovable.dev/features/cloud#secrets" target="_blank" rel="noreferrer">
          ...
          {lastChecked && (<span className="text-xs text-muted-foreground">
              Last checked {lastChecked.toLocaleTimeString()}
            </span>)}
        </div>

        {diag && (<div className="space-y-3 pt-2">
            <ul className="space-y-1.5 text-sm">
              {diag.checks.map((c, idx) => (<li key={idx} className="flex items-start gap-2">
                  {c.ok ? (<CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600"/>) : (<XCircle className="mt-0.5 h-4 w-4 shrink-0 text-destructive"/>)}
                  <span>
            ...
            {diag.ok && (<div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-900 dark:text-emerald-200">
                <CheckCircle2 className="mr-1 inline h-4 w-4"/>
                Key saved and verified. Venue lookups are live.
              </div>)}

            {!diag.ok && diag.remediation && (<div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-900 dark:text-amber-200">
