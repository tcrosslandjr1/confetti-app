...
function AdminTestimonialsPage() {
    ...
    async function remove(row: Testimonial) {
        if (!confirm(`Delete testimonial from ${row.name}?`))
            return;
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_22rem]">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {rows.map((r) => (<tr key={r.id} className="border-t border-border align-top">
                    ...
                    <td className="px-3 py-2">
                      {r.rating ? (<div className="flex items-center gap-0.5 text-amber-500">
                          {Array.from({ length: 5 }).map((_, i) => (<Star key={i} className={`h-3 w-3 ${i < (r.rating ?? 0) ? "fill-current" : "opacity-30"}`}/>))}
        ...
        <aside className="space-y-3 rounded-xl border border-border bg-card p-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="t-name" value={form.name} maxLength={100} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Ava Green"/>
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="t-username">Username / handle</Label>
            <Input id="t-username" value={form.username} maxLength={60} onChange={(e) => setForm({ ...form, username: e.target.value })} placeholder="@ava"/>
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="t-body">Body</Label>
            <Textarea id="t-body" value={form.body} maxLength={500} rows={4} onChange={(e) => setForm({ ...form, body: e.target.value })} placeholder="What they said about the product…"/>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="t-image" value={form.image_url ?? ""} maxLength={500} onChange={(e) => setForm({ ...form, image_url: e.target.value })} placeholder="https://…/avatar.jpg"/>
          </div>

          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1.5">
              <Label htmlFor="t-country">Country</Label>
              <Input id="t-country" value={form.country ?? ""} maxLength={60} onChange={(e) => setForm({ ...form, country: e.target.value })} placeholder="🇦🇺 Sydney"/>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="t-position">Position</Label>
              <Input id="t-position" type="number" value={form.position} onChange={(e) => setForm({ ...form, position: Number(e.target.value) || 0 })}/>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-0.5">
                {[1, 2, 3, 4, 5].map((n) => (<button key={n} type="button" onClick={() => setForm({ ...form, rating: n })} aria-label={`${n} star${n === 1 ? "" : "s"}`} className="p-0.5 text-amber-500">
                    <Star className={`h-5 w-5 ${form.rating !== null && n <= form.rating ? "fill-current" : "opacity-30"}`}/>
                  </button>))}
              </div>
              <button type="button" onClick={() => setForm({ ...form, rating: null })} className="text-xs text-muted-foreground underline">
                Clear
              </button>
            </div>
          </div>
