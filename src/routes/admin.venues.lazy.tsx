...
import { DollarSign, Edit3, Loader2, MailCheck, MapPin, Plus, RefreshCw, Search, Sparkles, Store, Trash2 } from "lucide-react";
...
export const Route = createLazyFileRoute("/admin/venues")({
...
function PriceLevel({ level }: {
    ...
    return (<span className="inline-flex items-center text-xs font-mono">
      {Array.from({ length: 4 }).map((_, i) => (<DollarSign key={i} className={`h-3 w-3 ${i < level ? "text-foreground" : "text-muted-foreground/30"}`}/>))}
    </span>);
}
...
function AdminVenuesPage() {
    ...
    const onRefreshMedia = async (v: Venue) => {
        ...
        try {
            ...
            if (r.error) {
                ...
                toast.success(`Refreshed ${v.name} — ${r.photos_added} photo${r.photos_added === 1 ? "" : "s"}, ${r.socials_found} social${r.socials_found === 1 ? "" : "s"}`);
            }
        }
        catch (e) {
    ...
    const onBulkRefresh = async () => {
        ...
        try {
            ...
            toast.success(`Refreshed ${r.venues_processed} venues — ${r.photos_added} photos, ${r.socials_found} socials`);
        }
        catch (e) {
    ...
    const load = async () => {
        ...
        if (error)
            toast.error("Couldn't load venues", { description: error.message });
        else
    ...
    return (<div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" disabled={bulkRefreshing} onClick={() => void onBulkRefresh()} title="Refresh photos + socials for up to 25 stale venues">
            {bulkRefreshing ? (<Loader2 className="mr-1 h-4 w-4 animate-spin"/>) : (<Sparkles className="mr-1 h-4 w-4"/>)}
            Run media refresh
          </Button>
          <Dialog open={adding} onOpenChange={setAdding}>
            ...
            <VenueDialog title="Add venue" description="Create a new venue. Customers will see it in search and the wizard." initial={EMPTY_DRAFT} submitLabel="Add venue" onSubmit={async (draft) => {
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative ml-auto min-w-[260px] flex-1 sm:flex-none">
          ...
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by name, neighborhood, city, category…" className="pl-9"/>
        </div>
      </div>

      {loading ? (<div className="grid place-items-center rounded-2xl border border-dashed border-border bg-card/50 p-10 text-sm text-muted-foreground">
          <Loader2 className="mb-2 h-5 w-5 animate-spin"/>
          Loading venues…
        </div>) : filtered.length === 0 ? (<div className="rounded-2xl border border-dashed border-border bg-card/50 p-10 text-center text-sm text-muted-foreground">
          {venues.length === 0
                ? "No venues yet — click Add venue to create the first one."
                : "No venues match your search."}
        </div>) : (<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((v) => (<article key={v.id} className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-5 shadow-card">
              ...
              {v.description && (<p className="line-clamp-3 text-sm text-foreground/85">{v.description}</p>)}

              <div className="mt-auto flex flex-wrap gap-2 pt-1">
                <Button size="sm" variant="outline" className="mr-auto" disabled={testingId === v.id} onClick={() => void onTestNotification(v)}>
                  {testingId === v.id ? (<Loader2 className="mr-1 h-3.5 w-3.5 animate-spin"/>) : (<MailCheck className="mr-1 h-3.5 w-3.5"/>)}
                  Test notification
                </Button>
                <Button size="sm" variant="outline" disabled={refreshingId === v.id} onClick={() => void onRefreshMedia(v)} title="Refresh photos + socials from Google + Firecrawl">
                  {refreshingId === v.id ? (<Loader2 className="mr-1 h-3.5 w-3.5 animate-spin"/>) : (<RefreshCw className="mr-1 h-3.5 w-3.5"/>)}
                  Refresh media
                </Button>
                <Button size="sm" variant="ghost" onClick={() => setEditing(v)}>
                  <Edit3 className="mr-1 h-3.5 w-3.5"/> Edit
                </Button>
                <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => void onDelete(v)}>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        {editing && (<VenueDialog key={editing.id} title="Edit venue" description="Update venue details. Changes save instantly." initial={editing} submitLabel="Save changes" onSubmit={async (draft) => {
...
function VenueDialog({ title, description, initial, submitLabel, onSubmit, }: {
    title: string;
    description: string;
    ...
    const update = <K extends keyof typeof EMPTY_DRAFT>(key: K, value: (typeof EMPTY_DRAFT)[K]) => setDraft((d) => ({ ...d, [key]: value }));
    useEffect(() => {
        void (async () => {
    ...
    const effectiveStaffEmail = (draft.staff_email && draft.staff_email.trim()) || linkedAdvertiser?.contact_email || "";
    return (<DialogContent className="flex max-h-[90dvh] w-[calc(100vw-1.5rem)] max-w-lg flex-col gap-4 overflow-hidden p-0 sm:w-full">
      ...
      <div className="grid gap-3 overflow-y-auto px-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="grid gap-1.5">
            ...
            <select id="price" value={draft.price_level} onChange={(e) => update("price_level", Number(e.target.value))} className="h-9 rounded-md border border-border bg-background px-3 text-sm">
        ...
        <div className="grid gap-1.5">
          ...
          <Textarea id="desc" value={draft.description ?? ""} onChange={(e) => update("description", e.target.value)} rows={4}/>
        </div>
        <div className="grid gap-1.5">
          <Label htmlFor="advertiser">Linked advertiser</Label>
          <select id="advertiser" value={draft.advertiser_id ?? ""} onChange={(e) => update("advertiser_id", e.target.value || null)} className="h-9 rounded-md border border-border bg-background px-3 text-sm">
            <option value="">— None —</option>
            {advertisers.map((a) => (<option key={a.id} value={a.id}>
                {a.business_name} ({a.contact_email})
              </option>))}
          </select>
          {linkedAdvertiser && !draft.staff_email?.trim() && (<p className="text-xs text-muted-foreground">
        ...
        <div className="grid gap-1.5">
          ...
          <Input id="staff_email" type="email" placeholder={linkedAdvertiser?.contact_email || "ops@venue.com"} value={draft.staff_email ?? ""} onChange={(e) => update("staff_email", e.target.value)}/>
      ...
      <DialogFooter className="border-t border-border bg-background px-6 py-4 pb-[max(env(safe-area-inset-bottom),1rem)]">
        <Button disabled={busy || !draft.name || !draft.category} onClick={async () => {
            setBusy(true);
            try {
