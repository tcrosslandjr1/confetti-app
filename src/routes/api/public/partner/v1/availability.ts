...
function buildSlots(date: string, partySize: number) {
  ...
  for (let h = 17; h < 23; h++) {
    for (const m of [0, 30]) {
      ...
      let extra: { remaining?: number; table_type?: string } = { table_type: m === 0 ? "indoor" : "patio" };
      if (seed === 0) status = "unavailable";
      else if (seed === 1) { status = "limited"; extra = { remaining: 1 }; }
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (auth.tier !== 3) return apiError("INVALID_TOKEN", "Availability requires Tier 3", { tier: auth.tier });

        const url = new URL(request.url);
        const date = url.searchParams.get("date");
        const partyRaw = url.searchParams.get("party_size");
        if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date)) return apiError("VALIDATION", "Invalid or missing 'date' (YYYY-MM-DD)");
        const party_size = Number(partyRaw);
        if (!Number.isInteger(party_size) || party_size < 1 || party_size > 50) return apiError("VALIDATION", "Invalid 'party_size'");
