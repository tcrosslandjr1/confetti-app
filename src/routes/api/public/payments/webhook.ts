...
const PROMO_SPEC: Record<
  string,
  { tier: string; duration: string; defaultTarget?: "venue" | "event" | "reel" }
> = {
...
async function activatePromo(args: {
  ...
  const targetType =
    (args.targetType as "venue" | "event" | "reel" | undefined) ?? spec.defaultTarget;
...
async function unlockVip(userId: string, days: number) {
  ...
  await sb
    .from("profiles")
    .update({ vip_until: newUntil, updated_at: new Date().toISOString() })
    .eq("id", userId);
  return newUntil;
}
...
async function handleCheckoutCompleted(session: any, env: StripeEnv) {
  ...
  if (priceId === "unlock_vip_30d_once") {
    ...
    await notifyUser(
      userId,
      "VIP Access unlocked 👑",
      `Your VIP perks are active until ${new Date(vipUntil).toLocaleDateString()}.`,
      "/passport",
    );
    ...
    await notifyUser(
      ...
      `Open the planner and tap "Premium plan" to use it.`,
      "/plan",
    );
  }
...
async function claimEvent(
  ...
  if ((insertErr as any).code !== "23505") {
    console.error("[webhook] claim insert failed", {
      eventId: event.id,
      type: event.type,
      env,
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          console.error("[webhook] claim failed, will let Stripe retry", {
            ...logCtx,
        ...
        try {
          ...
          try {
            ...
            console.error("[webhook] markFailed failed", {
              ...logCtx,
