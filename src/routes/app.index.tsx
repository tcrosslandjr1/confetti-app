        ...
        <div className="-mx-1 mt-3 flex snap-x snap-mandatory gap-3 overflow-x-auto px-5 pb-2">
          {(venues ?? []).map((v) => (
            <Link key={v.id} to="/venue/$id" params={{ id: v.id }} className="snap-start" onClick={() => trackEngagement("venue_tap", { venueId: v.id, venueName: v.name, source: "tonight_trending" })}>
              <div className="h-44 w-40 overflow-hidden rounded-2xl border border-border bg-muted transition-transform duration-200 active:scale-95 hover:scale-[1.03]">
      ...
      <section className="mt-6">
        ...
        <div className="mt-3 flex gap-3 overflow-x-auto px-5 pb-2">
          {(reels ?? []).map((r) => (
            <div
              key={r.id}
              className="relative h-56 w-36 shrink-0 overflow-hidden rounded-2xl bg-muted transition-transform duration-200 active:scale-95 hover:scale-[1.03]"
            >
              {r.thumbnail_url && (
                <img
                  src={r.thumbnail_url}
