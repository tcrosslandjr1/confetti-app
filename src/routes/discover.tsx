...
export const Route = createFileRoute("/discover")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse trending Confetti venues near you, in list or map view.",
      },
      { property: "og:title", content: "Discover Nearby — Confetti" },
      {
        property: "og:description",
        content: "Browse trending venues near you, in list or map view.",
      },
    ],
  }),
...
function DiscoverPage() {
  ...
  useEffect(() => {
    ...
    if (!filtered.some((r) => r.id === venueId)) {
      navigate({
        search: (prev: z.infer<typeof discoverSearchSchema>) => ({ ...prev, venueId: undefined }),
        replace: true,
      });
    }
  ...
  const setMapSelected = useCallback(
    (row: VenueRow | null) => {
      ...
      navigate({
        search: (prev: z.infer<typeof discoverSearchSchema>) => ({ ...prev, venueId: row.id }),
  ...
  useEffect(() => {
    if (venueId && viewParam !== "map") {
      navigate({
        search: (prev: z.infer<typeof discoverSearchSchema>) => ({ ...prev, view: "map" }),
        replace: true,
      });
    }
  }, [venueId, viewParam]);
                    ...
                    <div className="p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          {v.description && (
                            <div className="mt-0.5 truncate text-[11px] italic leading-snug text-ink/45">
                              {v.description}
                            </div>
                          )}
                        </div>
    ...
    <div className="absolute bottom-4 left-4 right-4 z-30 rounded-2xl border-2 border-ink bg-cream p-3 shadow-brut">
      ...
      >
        {row.photo ? (
          ...
          <div
            className={`h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 border-ink bg-gradient-to-br ${row.gradient ?? "from-slate-500 via-slate-700 to-slate-900"}`}
          >
      ...
      <a
        href={`https://www.tiktok.com/search?q=${encodeURIComponent(row.name)}`}
        target="_blank"
        rel="noopener noreferrer"
