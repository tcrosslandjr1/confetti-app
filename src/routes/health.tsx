    ...
    <div className="relative min-h-screen overflow-hidden bg-cream text-ink">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-2xl flex-col justify-center px-5 py-10">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <span className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-ink/60">
            v1.0
          </span>
        </div>
        ...
        <div className="rounded-[1.5rem] border-2 border-ink bg-cream/80 p-6 shadow-brut-lg backdrop-blur-xl sm:p-8">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-ink/60">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            {[
              ...
              >
                <div className="flex items-center gap-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.15em] text-ink/60">
          ...
          <div className="mt-6 space-y-2">
            {CHECKS.map(({ label, status, icon: Icon, detail, latency }, i) => (
              <div
                key={label}
                className="group flex items-center justify-between rounded-xl border-2 border-ink/15 bg-cream px-4 py-3 transition hover:border-ink hover:shadow-brut"
                style={{ animationDelay: `${i * 80}ms` }}
              >
                <div className="flex items-center gap-3">
                  <div className="grid h-9 w-9 place-items-center rounded-lg border-2 border-ink/15 bg-cream/60 text-ink/70 transition group-hover:border-ink group-hover:bg-coral group-hover:text-cream">
                    <Icon className="h-4 w-4" />
                  </div>
                  <div>
                    <div className="text-sm font-bold">{label}</div>
                    <div className="font-mono text-[10px] font-medium uppercase tracking-wider text-ink/55">
                      {detail}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="hidden font-mono text-[10px] font-bold text-ink/50 sm:inline">
                    {latency}ms
                  </span>
          ...
          <div className="mt-6 grid grid-cols-2 gap-x-4 gap-y-2.5 rounded-xl border border-dashed border-ink/25 bg-cream/40 p-4">
            {[
              ...
              <div key={label} className="flex items-center justify-between gap-2 text-xs">
                <span className="flex items-center gap-1.5 font-mono text-[10px] font-bold uppercase tracking-wider text-ink/55">
                  <Icon className="h-3 w-3" />
                  {label}
                </span>
                <span className="truncate font-mono text-[11px] font-bold text-ink">{value}</span>
              </div>
            ))}
          </div>
        </div>

        <p className="mt-5 text-center font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-ink/45">
