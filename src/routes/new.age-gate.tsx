    ...
    <Frame>
      <div style={{
        ...
        <div style={{
          ...
          <button onClick={() => navigate({ to: "/new/signup" })} style={backBtn()}>←</button>
          <BrandMark size={17} />
          <span style={{ width: 36 }} />
        </div>

        <Stamp color={TOKENS.accent2} rotate={-3} style={{ alignSelf: "flex-start" }}>quick check</Stamp>
        <h1 style={{
          fontFamily: TOKENS.display, fontWeight: 900,
          fontSize: 38, lineHeight: 0.92, letterSpacing: "-0.04em",
          margin: "10px 0 6px",
        }}>When were<br/>you born?</h1>
        <p style={{
          fontFamily: TOKENS.ui, fontSize: 13, fontWeight: 700, opacity: 0.6,
          margin: "0 0 22px",
        }}>Some venues are 21+. We tailor your night based on age.</p>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <DateBox label="year"  placeholder="YYYY" value={yyyy} onChange={setYyyy} max={4} flex={1.5} />
        </div>
        ...
        <Ticket color={TOKENS.paper} notch={false} style={{ padding: 12, marginTop: 18 }}>
          ...
          <div style={{
            fontFamily: TOKENS.ui, fontSize: 12, fontWeight: 700, marginTop: 4,
          }}>We never share your birthday. We use age only for venue eligibility.</div>
        </Ticket>

        <div style={{ flex: 1 }} />

        <div style={{ display: "flex", gap: 8 }}>
          <ChunkyButton variant="ghost" onClick={() => navigate({ to: "/new/parental-consent" })}>under 13</ChunkyButton>
          <ChunkyButton variant="accent" onClick={() => navigate({ to: "/new/hub" })} icon={Icons.arrow}>
            continue
          </ChunkyButton>
        </div>
      </div>
...
function DateBox({ label, placeholder, value, onChange, max, flex = 1 }: {
  label: string; placeholder: string; value: string; onChange: (v: string) => void; max: number; flex?: number;
}) {
  return (
    <div style={{ flex }}>
      <div style={{
      ...
      <input value={value} onChange={(e) => onChange(e.target.value.replace(/\D/g, ""))}
        inputMode="numeric" maxLength={max} placeholder={placeholder}
        style={{
          width: "100%", padding: "14px 12px", boxSizing: "border-box",
          border: `2.5px solid ${TOKENS.ink}`, borderRadius: 12,
          background: TOKENS.paper, textAlign: "center",
