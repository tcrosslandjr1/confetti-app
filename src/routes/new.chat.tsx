  ...
  async function send() {
    ...
    try {
      ...
      if (err.name !== "AbortError") {
        setMsgs((m) => [...m, { who: "confetti", text: `something went wrong — ${err.message ?? "try again"}` }]);
      }
    } finally {
      setStreaming(false);
    ...
    <Frame>
      <div style={{
        ...
        <div style={{ display: "flex", gap: 8, paddingTop: 12 }}>
          ...
          <ChunkyButton variant="accent" onClick={send} full={false} icon={Icons.arrow} disabled={streaming}>
            {streaming ? "…" : "send"}
          </ChunkyButton>
        </div>
      </div>
