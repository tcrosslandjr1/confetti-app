  ...
  return (
    <Frame>
      <div style={{
        ...
        <Stamp color={TOKENS.accent2} rotate={-3} style={{ alignSelf: "flex-start" }}>family mode</Stamp>
        <h1 style={{
          fontFamily: TOKENS.display, fontWeight: 900,
          fontSize: 38, lineHeight: 0.92, letterSpacing: "-0.04em",
          margin: "10px 0 16px", position: "relative", zIndex: 2,
        }}>Plan a<br/>family day.</h1>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            {PORTAL_OPTIONS.map((p) => (
              <Chip key={p} dense selected={portal === p} color={TOKENS.accent1}
                onClick={() => setPortal(p)}>{p}</Chip>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            {VIBES.map((v) => (
              <Chip key={v} dense selected={vibes.includes(v)} color={TOKENS.accent2}
                onClick={() => toggle(vibes, v, setVibes)}>{v}</Chip>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            {AGES.map((a) => (
              <Chip key={a} dense selected={ages.includes(a)} color={TOKENS.accent3}
                onClick={() => toggle(ages, a, setAges)}>{a}</Chip>
            ))}
          </div>
          ...
          <Ticket color={TOKENS.accent2} notch={false} style={{ padding: 14 }}>
            ...
            }}>WHAT WE'LL PRINT</div>
            <div style={{
              fontFamily: TOKENS.display, fontWeight: 900, fontSize: 15,
              letterSpacing: "-0.02em", marginTop: 6, lineHeight: 1.2,
            }}>3-stop {portal} day with {ages.join(" + ")}, paced for {vibes.join(" + ") || "easy"}.</div>
          </Ticket>
        </div>

        <div style={{ marginTop: 12, position: "relative", zIndex: 2 }}>
          <ChunkyButton variant="accent" onClick={() => navigate({ to: "/new/family-pass" })}
            icon={Icons.arrow}>print our day</ChunkyButton>
        </div>
      </div>
    </Frame>
  );
