  ...
  return (
    <Frame>
      <div style={{
        ...
        <Stamp color={TOKENS.accent2} rotate={-3} style={{ alignSelf: "flex-start" }}>verify · phone</Stamp>
        <h1 style={{
        ...
        <p style={{
          ...
          margin: "0 0 24px",
        }}>We texted (347) 555-0182 a 6-digit code.</p>

        {/* code boxes */}
        <div style={{ display: "flex", gap: 8, justifyContent: "space-between", marginBottom: 20 }}>
          {code.map((v, i) => (
            <input key={i} value={v} onChange={(e) => set(i, e.target.value)} inputMode="numeric" maxLength={1}
              style={{
        ...
        <ChunkyButton variant="accent" onClick={() => navigate({ to: "/new/email-verify" })}
          icon={Icons.arrow}>verify</ChunkyButton>
      </div>
    </Frame>
  );
}
