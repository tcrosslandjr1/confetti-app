...
export const Route = createFileRoute("/new/pizza-night")({ component: PizzaNightScreen });
...
const VENUES: Record<string, Venue> = {
  "jumbo-slice": { id: "jumbo-slice", name: "Jumbo Slice", tag: "Famous DC slice shop", price: "$", distance: "0.2 mi", rating: 4.6, color: TOKENS.accent1, tags: ["Legendary", "Late night", "Cash only"], description: "The OG Adams Morgan slice spot. Bigger than your head. Open until 3am on weekends." },
  "dessert-walk": { id: "dessert-walk", name: "Ice Cream Jubilee", tag: "Walkable ice cream", price: "$", distance: "0.3 mi", rating: 4.8, color: TOKENS.accent2, tags: ["Artisan", "DC flavors", "Walkable"], description: "Small-batch ice cream with DC-inspired flavors. The Honey Lavender is a must." },
  "il-canale": { id: "il-canale", name: "Il Canale", tag: "Sit-down Neapolitan", price: "$$", distance: "0.7 mi", rating: 4.7, color: TOKENS.accent1, tags: ["Wood-fired", "Georgetown", "Romantic"], description: "Authentic Neapolitan pizza in Georgetown. Wood-fired oven imported from Naples." },
  "dcd-games": { id: "dcd-games", name: "DC Draft", tag: "Bar with games", price: "$$", distance: "0.5 mi", rating: 4.5, color: TOKENS.accent3, tags: ["Shuffleboard", "Beer", "Groups"], description: "Shuffleboard, bocce, and 30+ craft beers. Great for groups of 4–10." },
  "pitango": { id: "pitango", name: "Pitango Gelato", tag: "Artisan gelato", price: "$", distance: "0.4 mi", rating: 4.9, color: TOKENS.accent2, tags: ["Artisan", "Organic", "Late night"], description: "The best gelato in DC, full stop. Try the salted caramel or the dark chocolate." },
  "pupatella": { id: "pupatella", name: "Pupatella", tag: "Award-winning Neapolitan", price: "$$$", distance: "1.1 mi", rating: 4.9, color: TOKENS.accent1, tags: ["James Beard", "Neapolitan", "Upscale casual"], description: "Nationally recognized Neapolitan pizza. BYOB and reservations recommended." },
  "off-the-record": { id: "off-the-record", name: "Off the Record", tag: "Speakeasy cocktail bar", price: "$$$", distance: "0.9 mi", rating: 4.8, color: TOKENS.ink, tags: ["Speakeasy", "Cocktails", "Political"], description: "Hidden beneath the Hay-Adams Hotel. Famous for political caricatures and strong pours." },
  "top-gate": { id: "top-gate", name: "Top of the Gate", tag: "Rooftop lounge", price: "$$$", distance: "1.3 mi", rating: 4.7, color: TOKENS.accent3, tags: ["Rooftop", "Views", "Sunset"], description: "Panoramic DC views from the Watergate. Sunset cocktails are unbeatable." },
};

const ITINERARIES: Itinerary[] = [
  {
    tier: "cheap", label: "Classic Night", emoji: "🍕", color: TOKENS.accent2,
    stops: [
      { time: "7:30 PM", venue: VENUES["jumbo-slice"], note: "The Adams Morgan classic" },
      { time: "9:00 PM", venue: VENUES["dessert-walk"], note: "Walk off the slice" },
    ],
    addons: ["Neighborhood stroll", "Street art photo walk", "Night cap at home"],
  },
  {
    tier: "mid", label: "Date Night", emoji: "🍷", color: TOKENS.accent1,
    stops: [
      { time: "7:00 PM", venue: VENUES["il-canale"], note: "Cozy Georgetown dinner" },
      { time: "9:30 PM", venue: VENUES["dcd-games"], note: "Shuffleboard + craft beers" },
      { time: "11:00 PM", venue: VENUES["pitango"], note: "Late-night gelato" },
    ],
    addons: ["Reservations at Il Canale", "Georgetown waterfront walk", "Gelato to-go"],
  },
  {
    tier: "high", label: "Premium Night", emoji: "👑", color: TOKENS.accent3,
    stops: [
      { time: "7:00 PM", venue: VENUES["pupatella"], note: "Best pizza in DC — reserve ahead" },
      { time: "9:30 PM", venue: VENUES["off-the-record"], note: "Speakeasy cocktails" },
      { time: "11:30 PM", venue: VENUES["top-gate"], note: "Rooftop nightcap" },
    ],
    addons: ["OpenTable reservation", "Resy for Off the Record", "Uber home"],
  },
];
...
function MapPreview({ stops, onExpand }: { stops: ItineraryStop[]; onExpand: () => void }) {
  return (
    <button onClick={onExpand} style={{ width: "100%", appearance: "none", cursor: "pointer", border: `2.5px solid ${TOKENS.ink}`, borderRadius: 16, background: "#f5e8d0", overflow: "hidden", boxShadow: `3px 3px 0 ${TOKENS.ink}`, padding: 0 }}>
...
function ShareSheet({ label, onClose }: { label: string; onClose: () => void }) {
  ...
  const link = `https://confettiplan.com/vibe/pizza-night`;
  return (
...
function FullMapScreen({ stops, onBack }: { stops: ItineraryStop[]; onBack: () => void }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 80, background: "#f5e8d0" }}>
      ...
      <div style={{ position: "absolute", top: 20, left: 20, right: 20 }}>
        <div style={{ background: TOKENS.paper, border: `3px solid ${TOKENS.ink}`, borderRadius: 16, padding: "12px 16px", boxShadow: `4px 4px 0 ${TOKENS.ink}` }}>
          ...
          <p style={{ fontFamily: TOKENS.display, fontSize: 16, fontWeight: 900, margin: "6px 0 4px" }}>Pizza Night Route</p>
  ...
  return (
    <Frame>
      ...
      <div className="cf-screen" style={{ minHeight: "100dvh", padding: "16px 20px 48px" }}>
        ...
        {screen === "overview" && (
          <div>
            <div style={{ background: `linear-gradient(135deg, ${TOKENS.accent1} 0%, #c93a1e 100%)`, border: `3px solid ${TOKENS.ink}`, borderRadius: 24, height: 200, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 72, marginBottom: 20, boxShadow: `5px 5px 0 ${TOKENS.ink}`, position: "relative", overflow: "hidden" }}>
            ...
            <p style={{ fontFamily: TOKENS.ui, fontSize: 12, fontWeight: 800, letterSpacing: ".14em", textTransform: "uppercase", color: TOKENS.accent1, margin: "0 0 6px" }}>vibe mode</p>
            <h1 style={{ fontFamily: TOKENS.display, fontSize: 30, fontWeight: 900, letterSpacing: "-0.03em", margin: "0 0 8px", lineHeight: 1.1 }}>Pizza Night</h1>
            <p style={{ fontFamily: TOKENS.ui, fontSize: 15, color: TOKENS.inkMuted, margin: "0 0 16px", lineHeight: 1.6 }}>
              From $2 slices to Neapolitan perfection — three ways to do pizza night, fully planned and ready to go.
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 28 }}>
              {["Comfort food", "Low-effort", "Casual date", "🍕 Wood-fired", "🥂 Optional drinks"].map(t => (
            ...
            <ChunkyButton onClick={handleGenerate} disabled={loading} variant="accent">
              {loading ? "Building your night…" : "Generate My Pizza Night 🍕"}
            </ChunkyButton>
          </div>
        )}
        ...
        {loading && screen === "overview" && (
          <div style={{ marginTop: 20 }}>
            <p style={{ fontFamily: TOKENS.ui, fontSize: 13, color: TOKENS.inkMuted, marginBottom: 16, textAlign: "center" }}>Finding the best pizza spots…</p>
        ...
        {screen === "itineraries" && !loading && (
          <div>
            <p style={{ fontFamily: TOKENS.ui, fontSize: 12, fontWeight: 800, letterSpacing: ".14em", textTransform: "uppercase", color: TOKENS.accent1, margin: "0 0 6px" }}>pizza night</p>
