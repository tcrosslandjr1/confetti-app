    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button onClick={() => navigate({ to: "/new/night" })} style={{
        ...
        <Stamp color={TOKENS.accent1} rotate={-3} style={{ alignSelf: "flex-start" }}>scan to check in</Stamp>
        ...
        <h1 style={{
          ...
          margin: "10px 0 4px",
        }}>Point your<br/>camera here.</h1>
        <p style={{
          fontFamily: TOKENS.ui, fontSize: 13, fontWeight: 700, opacity: 0.7,
          margin: 0,
        }}>Door staff or Confetti Cam frames this code.</p>

        {/* Viewfinder */}
        <div style={{
          flex: 1, display: "grid", placeItems: "center", margin: "20px 0",
        }}>
          <div style={{
            position: "relative",
            ...
            {/* fake QR mosaic */}
            <svg width="200" height="200" viewBox="0 0 20 20" style={{ imageRendering: "pixelated" }}>
              {Array.from({ length: 20 * 20 }).map((_, k) => {
                const x = k % 20, y = Math.floor(k / 20);
                const corner = (x < 3 && y < 3) || (x > 16 && y < 3) || (x < 3 && y > 16);
                const fill = corner ? 1 : Math.random() > 0.55 ? 1 : 0;
                return fill ? <rect key={k} x={x} y={y} width="1" height="1" fill={TOKENS.ink} /> : null;
              })}
            </svg>
            {/* corner brackets */}
            {([
              ...
              <span key={k} style={{
                ...
                borderColor: TOKENS.accent1, borderStyle: "solid",
                borderWidth: 0,
                ...Object.fromEntries(Object.entries(s).map(([kk, vv]) =>
                  typeof vv === "number" && kk.startsWith("border") ? [kk + "Width", vv + "px"] : [kk, vv]
        ...
        <div style={{
          ...
          }}>Lupa Notte · 8:00 PM</div>
        </div>

        <ChunkyButton variant="accent" onClick={() => navigate({ to: "/new/night" })} icon={Icons.check}>
          checked in
        </ChunkyButton>
      </div>
    </div>
