...
import {
  BrandMark, Chip, ChunkyButton, DotsBg, Frame, Icons, Stamp, Ticket, TOKENS,
...
const LOVES = ["pasta", "negronis", "live music", "rooftops", "tiny bars", "small plates", "disco", "wine", "bbq", "ramen"];
...
function TastePage() {
  ...
  const [sources, setSources] = useState<Record<string, boolean>>({ Spotify: true, Instagram: true });
  ...
  return (
    <Frame>
      <div style={{
        ...
        <Stamp color={TOKENS.accent2} rotate={-3} style={{ alignSelf: "flex-start" }}>your taste</Stamp>
        <h1 style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            {LOVES.map((v) => (
              <Chip key={v} dense selected={loves.includes(v)} color={TOKENS.accent2}
                onClick={() => toggle(loves, v, setLoves)}>{v}</Chip>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            {AVOIDS.map((v) => (
              <Chip key={v} dense selected={avoids.includes(v)} color={TOKENS.accent1}
                onClick={() => toggle(avoids, v, setAvoids)}>{v}</Chip>
            ))}
          </div>
          ...
          {SOURCES.map((s) => (
            <div key={s.name} style={{
              ...
              <button onClick={() => setSources((p) => ({ ...p, [s.name]: !p[s.name] }))} style={{
          ...
          <Ticket color={TOKENS.accent3} notch={false} style={{ padding: 14, marginTop: 14 }}>
            <div style={{ color: TOKENS.paper }}>
              ...
              }}>YOUR TASTE READING</div>
              <div style={{
                fontFamily: TOKENS.display, fontWeight: 900, fontSize: 15,
                letterSpacing: "-0.02em", marginTop: 6, lineHeight: 1.2,
              }}>Foodie disco-bro with a soft spot for tiny wine bars. We're keeping it real.</div>
            </div>
          </Ticket>
        </div>

        <div style={{ marginTop: 12 }}>
          <ChunkyButton variant="accent" onClick={() => navigate({ to: "/new/profile" })} icon={Icons.check}>
            save taste
          </ChunkyButton>
        </div>
      </div>
