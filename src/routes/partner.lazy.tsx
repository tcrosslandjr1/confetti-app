...
import { LayoutDashboard, Store, CalendarRange, UtensilsCrossed, CalendarDays, ClipboardList, ShoppingBag, BookOpen, BarChart3, CreditCard, Megaphone, LifeBuoy, Building2, Bell, Code2, Info, X } from "lucide-react";
...
function PartnerLayout() {
    ...
    return (<div className="min-h-screen bg-[hsl(var(--background))] text-foreground flex">
      {/* Sidebar */}
      <aside className="hidden md:flex w-64 flex-col border-r border-border/60 bg-card/40 backdrop-blur sticky top-0 h-screen">
        ...
        <div className="px-3 py-3 border-b border-border/60">
          ...
          <Button variant="ghost" size="sm" disabled className="w-full mt-1 justify-start text-xs text-muted-foreground" title="Venue claim flow coming soon">
            + Add venue
          </Button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-3 space-y-0.5">
          {NAV.map((item) => {
            ...
            return (<Link key={item.to} to={item.to} className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors ${active
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <main className="flex-1 p-4 md:p-8 max-w-[1400px] w-full mx-auto">
          {!bannerDismissed && (<div className="mb-6 flex items-start gap-3 rounded-lg border border-primary/20 bg-primary/5 px-4 py-3 text-sm">
              ...
              <button type="button" onClick={dismissBanner} className="text-muted-foreground hover:text-foreground" aria-label="Dismiss">
                <X className="h-4 w-4"/>
              </button>
            </div>)}
          <Outlet />
