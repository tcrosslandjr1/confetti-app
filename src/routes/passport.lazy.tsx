...
import { Award, Compass, Moon, Pizza, Users, Map as MapIcon, Crown, Gift, Sparkles, ArrowLeft, Flame, Ticket, TrendingUp, Lock, Share2, QrCode, Stamp, Zap, Calendar, ChevronRight, Star, Copy, Download, Check } from "lucide-react";
import { addConfetti, getConfetti, subscribeConfetti, getStamps, subscribeStamps, type PassportStamp } from "@/lib/loop-store";
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from "@/components/ui/alert-dialog";
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { PassportShareCard, encodePassport, type PassportShareData } from "@/components/passport/PassportShareCard";
...
export const Route = createLazyFileRoute("/passport")({
...
const SEED_STAMPS: PassportStamp[] = [
    {
        id: "seed-dc-1",
        city: "DC",
        theme: "Harbor Heatwave",
        date: "May 10",
        earnedAt: "2025-05-10T00:00:00Z",
    },
    {
        id: "seed-dc-2",
        city: "DC",
...
function PassportPage() {
    ...
    const currentTierIndex = Math.max(0, TIERS.findIndex((t, i) => confetti < (TIERS[i + 1]?.at ?? Infinity)));
    ...
    const shareUrl = typeof window !== "undefined" ? `${window.location.origin}/p/${shareCode}` : `/p/${shareCode}`;
      ...
      <div className="mx-auto max-w-5xl px-4 pt-6 sm:px-6 lg:px-8">
        ...
        <div className="relative mt-4 overflow-hidden rounded-3xl border-2 border-ink bg-gradient-vibe p-6 sm:p-8 text-cream shadow-brut-lg">
          ...
          <div className="relative flex items-center justify-between gap-2">
            ...
            <button type="button" onClick={() => setShareOpen(true)} className="inline-flex items-center gap-1 rounded-full border border-cream/40 bg-cream/10 px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-widest backdrop-blur hover:bg-cream/20">
          ...
          <div className="relative mt-5">
            <div className="h-3 w-full overflow-hidden rounded-full bg-ink/35 ring-1 ring-cream/10">
              <div className="relative h-full rounded-full bg-cream shadow-[0_0_14px_rgba(255,255,255,0.6)] transition-all duration-700" style={{ width: `${progress}%` }}>
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2 sm:gap-6">
          <div className="space-y-5 sm:space-y-6">
            ...
            <section className="mt-5 overflow-hidden rounded-2xl border-2 border-ink bg-card p-4 shadow-brut">
              ...
              <div className="mt-3 grid grid-cols-7 gap-1.5">
                {streakDays.map((on, i) => (<div key={i} className="flex flex-col items-center gap-1">
                    <div className={`grid h-9 w-full place-items-center rounded-lg border-2 text-[11px] font-bold transition-transform hover:-translate-y-0.5 ${on
            ...
            <section className="mt-5">
              <div className="flex items-end justify-between">
                ...
                {nextTier && (<span className="font-mono text-[10px] uppercase tracking-widest text-ink/60">
            ...
            return (<div key={t.name} className={`relative flex flex-col items-center gap-1.5 rounded-2xl border-2 p-3 text-center transition-all ${isCurrent
                    ...
                    : reached
                      ...
                      <div className="font-display text-xs font-extrabold leading-none">
                        {t.name}
                      </div>
                      <div className={`font-mono text-[9px] uppercase tracking-widest ${isCurrent ? "opacity-90" : "opacity-70"}`}>
                        {t.at}
                      </div>
                      {isCurrent && (<span className="absolute -top-1.5 right-1.5 grid h-4 w-4 place-items-center rounded-full border-2 border-ink bg-coral">
            ...
            <section className="mt-6">
              <div className="flex items-end justify-between">
                ...
                <Link to="/portal/activity" className="inline-flex items-center gap-0.5 font-mono text-[10px] font-bold uppercase tracking-widest text-ink/60 hover:text-ink">
              ...
              <div className="mt-3 -mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {stamps.map((s) => (<div key={s.id} className={`relative snap-start shrink-0 overflow-hidden rounded-2xl border-2 p-4 transition-all ${s.earned
                ? "border-ink bg-cream shadow-brut"
                : "border-ink/30 bg-card/40 opacity-60 grayscale"}`} style={{ width: 140 }}>
                    <div className="pointer-events-none absolute -right-6 -top-6 h-20 w-20 rounded-full bg-coral/10 blur-xl"/>
                    {s.earned && (<span className="absolute right-2 top-2 rounded-full border border-ink bg-coral px-1.5 py-0.5 font-mono text-[8px] font-bold uppercase tracking-widest text-cream">
                        New
                      </span>)}
                    {!s.earned && (<span className="absolute right-2 top-2 grid h-5 w-5 place-items-center rounded-full border border-ink/40 bg-cream/80 text-ink/60">
                        <Lock className="h-2.5 w-2.5"/>
                      </span>)}
                    <div className={`mx-auto grid h-20 w-20 -rotate-6 place-items-center rounded-full border-4 border-dashed bg-cream text-center ${s.earned ? "border-coral" : "border-ink/30"}`}>
                      <div>
                        <div className={`font-mono text-[8px] font-bold uppercase tracking-widest ${s.earned ? "text-coral" : "text-ink/40"}`}>
                          Confetti
                        </div>
                        <div className="mt-0.5 font-display text-[11px] font-extrabold leading-none text-ink">
                          {s.city}
                        </div>
                        <div className="mt-0.5 font-mono text-[7px] uppercase text-ink/60">
                          {s.date}
                        </div>
                      </div>
                    </div>
                ...
                <div className="grid snap-start shrink-0 place-items-center rounded-2xl border-2 border-dashed border-ink/30 bg-card/40 p-4 text-center" style={{ width: 140 }}>
                  <div>
            ...
            <section className="mt-6">
              ...
              <div className="mt-3 grid grid-cols-3 gap-3">
                {badges.map((b) => (<div key={b.id} className={`group relative flex flex-col items-center gap-2 rounded-2xl border-2 p-3 text-center transition-all ${b.unlocked
                ? "border-ink bg-card shadow-brut hover:-translate-y-0.5"
                : "border-dashed border-ink/30 bg-card/50"}`}>
                    {b.unlocked && (<span className="absolute -top-1.5 -right-1.5 rounded-full border-2 border-ink bg-gold px-1.5 py-0.5 font-mono text-[8px] font-bold uppercase tracking-widest text-ink">
                        new
                      </span>)}
                    <span className={`relative grid h-12 w-12 place-items-center rounded-full ${b.unlocked
                ? "bg-gradient-vibe text-cream shadow-[0_4px_12px_rgba(0,0,0,0.18)]"
                : "bg-muted text-muted-foreground"}`}>
                      <b.icon className="h-5 w-5"/>
                      {!b.unlocked && (<span className="absolute -bottom-1 -right-1 grid h-5 w-5 place-items-center rounded-full border-2 border-background bg-ink text-cream">
                          <Lock className="h-2.5 w-2.5"/>
                        </span>)}
                    </span>
                    <div className={`font-display text-xs font-bold leading-tight ${b.unlocked ? "" : "text-ink/60"}`}>
                      {b.label}
                    </div>
          ...
          <div className="space-y-5 sm:space-y-6">
            ...
            <section className="mt-6 relative overflow-hidden rounded-2xl border-2 border-ink bg-ink p-5 text-cream shadow-brut-lg">
              ...
              <div className="relative flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <Link to="/partners" className="mt-3 inline-flex items-center gap-1 rounded-full border-2 border-cream/40 bg-cream/10 px-3 py-1.5 font-mono text-[10px] font-bold uppercase tracking-widest hover:bg-cream/20">
            ...
            return (<li key={r.id} className={`relative overflow-hidden rounded-xl border-2 border-ink bg-card p-3 shadow-brut transition-transform ${can ? "hover:-translate-y-0.5" : ""}`}>
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex min-w-0 items-center gap-3">
                          <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-full ${can ? "bg-gradient-vibe text-cream" : "bg-muted text-muted-foreground"}`}>
                    ...
                    return (<button type="button" disabled={!can || isClaimed || !!pending} onClick={() => setPending(r)} className="shrink-0 rounded-full border-2 border-ink bg-coral px-3 py-1.5 font-mono text-[10px] font-bold uppercase tracking-widest text-cream shadow-brut transition-transform hover:-translate-y-0.5 disabled:translate-y-0 disabled:opacity-40 disabled:shadow-none">
                              {isClaimed ? "Claimed" : can ? "Redeem" : "Locked"}
                            </button>);
                })()}
                      </div>
                      {!can && (<div className="mt-2.5">
            ...
            {/* Claimed rewards */}
            {claimed.length > 0 && (<section className="mt-6">
                <div className="flex items-end justify-between">
                  <h2 className="flex items-center gap-2 font-display text-lg font-bold">
                ...
                <ul className="mt-3 space-y-2">
                  {claimed.map((c) => (<li key={c.code} className="flex items-center justify-between gap-3 rounded-xl border-2 border-ink bg-card p-3 shadow-brut">
                      <div className="min-w-0">
                        <div className="font-display text-sm font-bold leading-tight">
                          {c.label}
                        </div>
                    ...
                    navigator.clipboard.writeText(c.code).then(() => {
                        ...
                        {c.code}
                        {copiedCode === c.code ? (<Check className="h-3.5 w-3.5 text-coral"/>) : (<Copy className="h-3.5 w-3.5 text-ink/50 group-hover:text-coral"/>)}
                      </button>
                    </li>))}
                </ul>
              </section>)}
        ...
        ].map((a, i) => (<li key={i} className="flex items-start gap-3 rounded-xl border-2 border-ink/10 bg-card p-3 transition-colors hover:border-ink/30">
                    <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full ${a.earn ? "bg-coral/15 text-coral" : "bg-ink/10 text-ink"}`}>
        ...
        <div className="mt-6 flex items-center justify-between gap-3 rounded-2xl border-2 border-dashed border-ink/30 bg-cream/60 p-4">
          ...
          <Link to="/scan" className="rounded-full border-2 border-ink bg-background px-3 py-1.5 font-mono text-[10px] font-bold uppercase tracking-widest text-ink hover:bg-ink hover:text-cream">
            Scan
          </Link>
        </div>
      </div>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction disabled={redeeming} onClick={handleConfirmRedeem}>
              {redeeming ? (<span className="inline-flex items-center gap-2">
        ...
        <DialogContent className="max-w-md border-2 border-ink bg-card p-0 shadow-brut-lg">
          ...
          <div className="px-6 pb-6 pt-4">
            <div className="flex items-center gap-2 rounded-xl border-2 border-ink bg-cream/60 px-3 py-2">
              ...
              <button type="button" onClick={handleCopyLink} className="inline-flex items-center gap-1 rounded-full border-2 border-ink bg-cream px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-widest text-ink shadow-brut hover:-translate-y-0.5 transition-transform">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <button type="button" onClick={handleDownloadImage} className="inline-flex items-center justify-center gap-1.5 rounded-full border-2 border-ink bg-cream px-3 py-2 font-mono text-[11px] font-bold uppercase tracking-widest text-ink shadow-brut hover:-translate-y-0.5 transition-transform">
                <Download className="h-3.5 w-3.5"/> Download
              </button>
              <button type="button" onClick={handleNativeShare} className="inline-flex items-center justify-center gap-1.5 rounded-full border-2 border-ink bg-coral px-3 py-2 font-mono text-[11px] font-bold uppercase tracking-widest text-cream shadow-brut hover:-translate-y-0.5 transition-transform">
                <Share2 className="h-3.5 w-3.5"/> Share
              </button>
            </div>
          </div>
...
function StatTile({ icon: Icon, value, label, tint, }: {
    ...
    return (<div className="flex flex-col items-center justify-center gap-1 rounded-2xl border-2 border-ink bg-card p-3 shadow-brut">
      <span className={`grid h-7 w-7 place-items-center rounded-full ${tint === "coral" ? "bg-coral/15 text-coral" : "bg-ink/10 text-ink"}`}>
