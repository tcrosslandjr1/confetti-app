      ...
      <div className="mx-auto max-w-md px-4 pt-6">
        ...
        <div className="mt-4 rounded-3xl border-2 border-ink bg-gradient-vibe p-6 text-cream shadow-brut-lg">
          ...
          <div className="mt-2 flex items-end justify-between gap-3">
            ...
            <div className="text-right">
              <div className="font-display text-3xl font-extrabold">{confetti.toLocaleString()}</div>
              <div className="font-mono text-[10px] uppercase tracking-widest opacity-90">Confetti</div>
            </div>
          </div>
          <div className="mt-4">
            ...
            <div className="mt-1 flex justify-between font-mono text-[9px] uppercase tracking-widest opacity-80">
              ...
              <span>{nextLevelAt - confetti} to L{level + 1}</span>
            </div>
          </div>
        </div>
        ...
        <section className="mt-6">
          ...
          <div className="mt-3 grid grid-cols-3 gap-3">
            {BADGES.map((b) => (
              <div key={b.id} className={`flex flex-col items-center gap-2 rounded-2xl border-2 p-4 text-center ${b.unlocked ? "border-ink bg-card shadow-brut" : "border-dashed border-ink/30 bg-card/50 opacity-60"}`}>
                <span className={`grid h-12 w-12 place-items-center rounded-full ${b.unlocked ? "bg-gradient-vibe text-cream" : "bg-muted text-muted-foreground"}`}>
          ...
          <ul className="mt-3 space-y-2">
            {[
              ...
              <li key={i} className="flex items-start gap-3 rounded-xl border border-border bg-card p-3">
                <span className="grid h-9 w-9 place-items-center rounded-full bg-coral/10 text-coral"><Sparkles className="h-4 w-4" /></span>
                <div>
        ...
        {/* Rewards */}
        <section className="mt-6">
          <h2 className="font-display text-lg font-bold flex items-center gap-2"><Gift className="h-5 w-5" /> Redeem Confetti</h2>
          <ul className="mt-3 space-y-2">
            {REWARDS.map((r) => {
              const can = confetti >= r.cost;
              return (
                <li key={r.id} className="flex items-center justify-between rounded-xl border-2 border-ink bg-card p-3 shadow-brut">
                  <div>
                    <div className="font-display text-sm font-bold">{r.label}</div>
                    <div className="font-mono text-[10px] uppercase tracking-widest text-ink/60">{r.cost} Confetti</div>
                  </div>
                  <button disabled={!can} className="rounded-full border-2 border-ink bg-coral px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-widest text-cream disabled:opacity-40">
                    {can ? "Redeem" : "Locked"}
                  </button>
                </li>
              );
