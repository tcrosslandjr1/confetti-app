  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const unlockedMap = new Map(
        ((uaRes.data ?? []) as { achievement_id: string; unlocked_at: string | null }[]).map(
          (r) => [r.achievement_id, r.unlocked_at],
        ),
      );
      const merged: Achievement[] = (
        (achRes.data ?? []) as Omit<Achievement, "unlocked" | "unlocked_at">[]
      )
        .map((d) => ({
          ...d,
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-muted-foreground">
              Badges you've earned plus exactly how to unlock the next ones.
            </p>
          </div>
        </div>
      </header>
          ...
          <Stat
            ...
            value={
              totals.unlocked === totals.total
                ? "All done!"
                : `${totals.total - totals.unlocked} to go`
            }
            hint="Keep exploring"
          />
        </div>
      ...
      {loading ? (
        ...
        <>
          {tab !== "locked" && unlocked.length > 0 && (
            <section>
              <h2 className="mb-3 flex items-center gap-2 font-display text-xl font-bold">
                <CheckCircle2 className="h-5 w-5 text-emerald-500" /> Unlocked
              </h2>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {a.unlocked_at && (
                          <p className="mt-2 font-mono text-[10px] uppercase tracking-widest text-emerald-600 dark:text-emerald-400">
          ...
          {tab !== "unlocked" && locked.length > 0 && (
            <section>
              <h2 className="mb-3 flex items-center gap-2 font-display text-xl font-bold">
                <Lock className="h-5 w-5 text-muted-foreground" /> Up next
              </h2>
              <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {locked.map((a) => (
                  <li
                        ...
                        <div className="mt-3 rounded-lg border border-dashed border-border bg-background/60 p-2.5">
                          <div className="flex items-start gap-2">
                            ...
                            <p className="text-[11px] leading-snug text-foreground/80">
                              <span className="font-semibold">How to unlock:</span> {unlockHint(a)}
                            </p>
                          </div>
                        </div>
                      </div>
