...
export const Route = createLazyFileRoute("/promoter")({
  component: PromoterLayout,
});

function PromoterLayout() {
    return (<div className="min-h-screen bg-gradient-to-b from-orange-50/40 via-background to-background">
...
function NavTab({ to, icon, label }: {
    ...
    return (<Link to={to} activeOptions={{ exact: to === "/promoter" }} className={cn("px-3 py-2 rounded-md hover:bg-muted/60 inline-flex items-center gap-2")} activeProps={{ className: "bg-primary/10 text-primary font-medium" }}>
      {icon}
