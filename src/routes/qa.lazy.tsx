    ...
    for (const s of SUITES) {
        try {
            for (const r of s.run())
                rows.push({ suiteId: s.id, suiteName: s.name, result: r });
        }
        catch (e) {
            rows.push({
                suiteId: s.id,
...
function QAPage() {
    ...
    const grouped = useMemo(() => {
        ...
        for (const r of rows) {
            if (filter !== "all" && r.result.status !== filter)
                continue;
            const k = `${r.suiteId}:${r.suiteName}`;
            if (!map.has(k))
                map.set(k, { id: r.suiteId, name: r.suiteName, results: [] });
    ...
    return (<main className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="flex flex-wrap gap-3 items-center mb-6">
        <button className="px-4 py-2 rounded bg-primary text-primary-foreground font-semibold" onClick={() => setRows(runAll())}>
          Run all suites
        </button>
        {rows && (<>
            ...
            {(["all", "pass", "fail", "skip"] as const).map((f) => (<button key={f} onClick={() => setFilter(f)} className={`px-3 py-1 text-xs rounded border ${filter === f ? "bg-foreground text-background" : ""}`}>
                {f}
              </button>))}
          </>)}
      </div>
      ...
      {grouped.map((g) => (<section key={g.id} className="mb-6 border rounded-lg overflow-hidden">
          ...
          <ul className="divide-y">
            {g.results.map((r) => (<li key={r.id} className="px-4 py-2 flex gap-3 items-start text-sm">
                <span className={`shrink-0 inline-block w-14 text-center rounded text-xs font-bold py-0.5 ${r.status === "pass"
