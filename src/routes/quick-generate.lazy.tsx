...
export const Route = createLazyFileRoute("/quick-generate")({
...
function QuickGenerate() {
    ...
    if (phase === "gen") {
        ...
        return (<div className="fixed inset-0 grid place-items-center bg-background p-6">
        <div className="flex w-full max-w-md flex-col items-center text-center">
          ...
          <ul className="mt-6 w-full space-y-2 text-sm">
            {lines.map((l, i) => (<li key={l} className={`transition-all duration-500 ${i < statusStep ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}>
    ...
    return (<div className="mx-auto max-w-2xl space-y-6 p-4 pb-24 sm:p-6">
      <header>
      ...
      <ul className="space-y-3">
        {stops.map((s, i) => (<li key={s.name + i} className="rounded-2xl border border-border bg-card p-4 shadow-card">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {openSwap === i && (<div className="mt-3 space-y-2 border-t border-border pt-3 animate-fade-in">
                    {s.alternatives.map((a) => (<button key={a.name} onClick={() => swap(i, a)} className="flex w-full items-center gap-3 rounded-xl border border-border bg-background p-2.5 text-left hover:bg-muted">
      ...
      <section className="rounded-2xl border border-border bg-card p-4 shadow-card">
        ...
        {showTweaks && (<div className="mt-4 space-y-3 animate-fade-in">
            <div className="flex flex-wrap gap-2">
              {TWEAKS.map((t) => (<button key={t} onClick={() => setTweak(tweak ? `${tweak}, ${t}` : t)} className="rounded-full border border-border bg-background px-3 py-1.5 text-xs font-semibold hover:bg-muted">
                  {t}
                </button>))}
            </div>
...
function RoutePreview({ stops }: {
    ...
    return (<section className="rounded-2xl border border-border bg-card p-3 shadow-card">
      <div className="overflow-hidden rounded-xl">
        <ConfettiMap stops={stops.map((s, i) => ({ id: `qg-${i}`, name: s.name, area: s.area }))} fallbackCity={stops[0]?.area || "Los Angeles"} height={140} interactive={false}/>
      </div>
