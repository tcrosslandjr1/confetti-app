...
function ScanPage() {
    ...
    return (<main className="min-h-screen bg-cream text-ink p-6">
      <div className="mx-auto max-w-md space-y-6">
        ...
        <div className="rounded-2xl border-2 border-ink bg-cream p-4 shadow-brut">
          <div className="relative aspect-square w-full overflow-hidden rounded-xl border-2 border-ink bg-ink">
            ...
            {status !== "scanning" && (<div className="absolute inset-0 grid place-items-center bg-ink/80 text-cream">
                {status === "unsupported" ? (<div className="px-4 text-center text-xs">
                    <AlertTriangle className="mx-auto mb-2 h-6 w-6"/>
                    Your browser doesn't support camera QR scanning. Use the manual code box below.
                  </div>) : status === "error" ? (<div className="px-4 text-center text-xs">
                    <AlertTriangle className="mx-auto mb-2 h-6 w-6"/>
                    {errorMsg || "Couldn't open the camera."}
                  </div>) : (<Camera className="h-10 w-10"/>)}
              </div>)}
            {status === "scanning" && (<div className="pointer-events-none absolute inset-0 grid place-items-center">
          ...
          <button type="button" onClick={startScan} disabled={status === "scanning"} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-ink bg-coral px-4 py-3 text-sm font-bold text-cream shadow-brut transition-pop hover:-translate-y-0.5 disabled:opacity-60 disabled:hover:translate-y-0">
            <Camera className="h-4 w-4"/>
            {status === "scanning" ? "Scanning…" : "Start camera"}
          </button>
        </div>

        <form onSubmit={submitManual} className="rounded-2xl border-2 border-ink bg-cream p-4 shadow-brut">
          ...
          <input type="text" value={manual} onChange={(e) => setManual(e.target.value)} placeholder="https://confetti.app/check-in?loop=…&stop=…" className="mt-2 w-full rounded-xl border-2 border-ink bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-coral"/>
          <button type="submit" className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-ink bg-cream px-4 py-2.5 text-sm font-bold shadow-brut transition-pop hover:-translate-y-0.5 hover:bg-gold">
            <KeyRound className="h-4 w-4"/> Check in
          </button>
          {errorMsg && status !== "error" && status !== "unsupported" && (<p className="mt-2 text-xs text-coral">{errorMsg}</p>)}
        </form>
      </div>
    </main>);
}
