...
function TasteTuner() {
  ...
  const onPointerUp = () => {
    if (Math.abs(drag.x) > 100) swipe(drag.x > 0 ? "like" : "pass");
  ...
  if (done) {
    return (
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-md flex-col items-center justify-center gap-6 p-6 text-center">
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-md flex-col p-4">
      ...
      <div className="relative mx-auto h-[440px] w-full max-w-sm flex-1">
        {next && (
          <div className="absolute inset-0 scale-95 rounded-3xl border border-border bg-card opacity-60 shadow-card" />
        )}
        <div
