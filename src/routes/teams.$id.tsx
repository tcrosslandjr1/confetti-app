...
function TeamEventPage() {
  ...
  const dietary = attendees
    .filter((a) => a.dietary?.trim())
    .map((a) => `${a.name || a.email}: ${a.dietary}`);
  ...
  const addAttendee = async () => {
    ...
    const { error } = await supabase
      .from("corporate_attendees")
      .insert({ event_id: id, email, role: "attendee" });
    if (error) {
  ...
  if (user && event.owner_id !== user.id) {
    return (
      <>
        <SiteHeader />
        <main className="mx-auto max-w-3xl px-4 py-16 text-center">
          <h1 className="font-display text-3xl font-bold">You don't have access to this event</h1>
        </main>
        <SiteFooter />
      </>
    );
          ...
          <header className="mt-3 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 font-mono text-xs uppercase tracking-widest text-ink/60">
                {event.org_name} · {new Date(event.starts_at).toLocaleDateString()} · {days}{" "}
                {days === 1 ? "day" : "days"} · {event.headcount} guests
              </p>
            </div>
            ...
            {tab === "overview" && (
              <div className="grid gap-4 lg:grid-cols-3">
                ...
                <Stat
                  icon={CheckCircle2}
                  label="RSVPs in"
                  value={`${responded.length}/${attendees.length || event.headcount}`}
                  hint={`${yesCount} yes`}
                />
                <Stat
                  icon={Wallet}
                  label="Per person"
                  value={`$${(event.budget_per_person_cents / 100).toFixed(0)}`}
                  hint={`Cap: $${((event.budget_per_person_cents * event.headcount) / 100).toLocaleString()}`}
                />
                <div className="rounded-2xl border-2 border-ink bg-cream p-5 shadow-brut lg:col-span-3">
                  ...
                  <div className="mt-3 flex gap-2">
                    <Link
                      to="/concierge/chat"
                      className="inline-flex h-10 items-center rounded-full border-2 border-ink bg-ink px-4 font-mono text-xs font-bold uppercase tracking-widest text-cream shadow-brut"
                    >
                      Draft with Concierge
                    </Link>
                    <Button variant="outline" onClick={() => setTab("attendees")}>
                      Manage attendees
                    </Button>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap items-end gap-2 rounded-2xl border-2 border-ink bg-cream p-4 shadow-brut">
                  ...
                  <Button onClick={addAttendee}>
                    <Mail className="mr-2 h-4 w-4" /> Add
                  </Button>
                </div>
                      ...
                      {attendees.length === 0 && (
                        <tr>
                          <td colSpan={4} className="px-4 py-8 text-center text-ink/60">
                            No attendees yet — add some above.
                          </td>
                        </tr>
                      )}
                      {attendees.map((a) => (
                        <tr key={a.id} className="border-b border-ink/10 last:border-0">
                          <td className="px-4 py-3">
                            <div className="font-bold">{a.name || a.email}</div>
                            <div className="text-xs text-ink/60">{a.email}</div>
                          </td>
                          <td className="px-4 py-3">
                            <RsvpBadge status={a.rsvp_status} />
                          </td>
                          <td className="px-4 py-3">
                            <RsvpLink token={a.rsvp_token} />
                          </td>
            ...
            {tab === "budget" && (
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Stat
                  ...
                  value={`$${((event.budget_per_person_cents * event.headcount) / 100).toLocaleString()}`}
                  hint={`${event.headcount} × cap`}
                />
                <div className="rounded-2xl border-2 border-ink bg-cream p-5 shadow-brut sm:col-span-2">
                  <p className="text-sm text-ink/70">
                    Once stops are booked we'll show actual spend vs. cap here.
                  </p>
                </div>
              </div>
            )}
            ...
            {tab === "share" && (
              <div className="rounded-2xl border-2 border-ink bg-cream p-6 shadow-brut">
                ...
                <p className="mt-1 text-sm text-ink/70">
                  Each attendee has a unique RSVP link in the Attendees tab. They don't need an
                  account to respond.
                </p>
              </div>
            )}
...
function RsvpBadge({ status }: { status: RsvpStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 font-mono text-[10px] font-bold uppercase tracking-widest ${m.cls}`}
    >
...
function RsvpLink({ token }: { token: string }) {
  const url =
    typeof window !== "undefined" ? `${window.location.origin}/rsvp/${token}` : `/rsvp/${token}`;
  const copy = () => {
