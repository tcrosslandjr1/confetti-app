...
import { completeItinerary, getItinerary, updateItinerary, updateStop, type Itinerary, type Stop } from "@/lib/itineraries";
...
function Passport() {
  ...
  const [overall, setOverall] = useState<{ rating: number; review: string }>({ rating: 0, review: "" });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setStops(data.stops);
        setOverall({ rating: data.itinerary.overall_rating ?? 0, review: data.itinerary.overall_review ?? "" });
  ...
  if (loading) return <div className="grid min-h-screen place-items-center text-sm text-muted-foreground">Loading passport…</div>;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
        <Link to="/trips/$id" params={{ id }} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
            ...
            <div className="grid h-24 w-24 shrink-0 -rotate-12 place-items-center rounded-full border-4 border-primary/70 text-center">
              <div>
                ...
                <p className="mt-1 text-[10px] font-bold uppercase leading-tight text-primary">
                  Stamped<br />{stamped.toLocaleDateString(undefined, { month: "short", day: "numeric" })}
                </p>
              </div>
            </div>
          </div>
        ...
        <p className="text-sm text-muted-foreground">Your ratings train your algorithm for next time.</p>
          ...
          {stops.map((s, i) => (
            <li key={s.id} className="rounded-2xl border border-border bg-card p-5 shadow-card">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Stop {i + 1}</p>
                  <h3 className="font-display text-lg font-bold">{s.name}</h3>
                </div>
                <StarPicker value={s.user_rating ?? 0} onChange={(v) => s.id && rateStop(s.id, v)} />
              </div>
              <textarea
                value={s.user_review ?? ""}
        ...
        <div className="mt-10 rounded-2xl border border-border bg-card p-6 shadow-card">
          ...
          <div className="mt-4 flex items-center gap-3">
            <StarPicker value={overall.rating} onChange={(v) => setOverall((p) => ({ ...p, rating: v }))} size="lg" />
            <span className="text-sm text-muted-foreground">{overall.rating ? `${overall.rating}/5` : "Tap a star"}</span>
          </div>
          <textarea
...
function StarPicker({ value, onChange, size = "md" }: { value: number; onChange: (v: number) => void; size?: "md" | "lg" }) {
  const sz = size === "lg" ? "h-7 w-7" : "h-5 w-5";
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((n) => (
        <button key={n} type="button" onClick={() => onChange(n)} aria-label={`${n} star`}>
          <Star className={`${sz} ${n <= value ? "fill-amber-400 text-amber-400" : "text-muted-foreground/40"}`} />
        </button>
      ))}
    </div>
  );
