...
function VibePlansPage() {
    ...
    async function build() {
        ...
        try {
            ...
            if (customVibe.trim())
                tweaks.push(`custom vibe: ${customVibe.trim()}`);
            ...
            if (resolved)
                tweaks.push(buildCategoryDirective(resolved, freeText));
            else if (freeText.trim())
                tweaks.push(`user said: "${freeText.trim().slice(0, 200)}"`);
            // Filter toggles
            if (familyFriendly)
            ...
            if (avoidAlcohol)
                tweaks.push("avoid alcohol-centric venues; prefer dry or low-ABV options");
            if (indoorOnly)
                tweaks.push("indoor stops only — no outdoor venues");
            if (budgetFriendly)
                tweaks.push("budget-friendly: prioritize affordable picks under $25/person/stop");
            // Brunch Baddies-aware add-ons
            const isBrunchBaddies = catIds.includes("brunch_baddies") || (vibe && BRUNCH_BADDIES_TRIGGER_IDS.has(vibe.id));
            if (isBrunchBaddies) {
                tweaks.push("Brunch Baddies daytime-first plan: cute fits, mimosas, rooftop or waterfront brunch, aesthetic plates, photo/flower walls, daytime music. No clubs or late-night venues.");
                if (postBrunchDayParty)
                    tweaks.push("add an optional post-brunch day party stop");
            }
      ...
      <div className="space-y-5 px-5">
        ...
        <Card className="p-4">
          ...
          <div className="mt-2 flex gap-2">
            <Input placeholder="Nashville, Memphis, Tennessee…" value={cityQuery} onChange={(e) => setCityQuery(e.target.value)} onKeyDown={(e) => {
            if (e.key === "Enter")
                handleCitySearch();
        }}/>
          ...
          {pendingState ? (<div className="mt-3 rounded-lg border border-dashed p-3">
              <p className="text-sm">
                Which city in <span className="font-semibold">{pendingState.label}</span>?
              </p>
              <div className="mt-2 flex flex-wrap gap-2">
                {pendingState.cities.map((c) => (<button key={c.slug} onClick={() => pickCity(c)} className="rounded-full border bg-card px-3 py-1 text-xs hover:bg-accent">
                    {c.label}
                  </button>))}
              </div>
            </div>) : null}

          {city ? (<div className="mt-3 flex items-center gap-2 text-xs">
              <Badge variant="default">{city.label}</Badge>
              {waterfront?.hasWaterfront ? (<Badge variant="secondary" className="gap-1">
        ...
        <Card className="space-y-3 p-4">
          <div className="flex items-center justify-between">
            ...
            {selectedCats.length ? (<button onClick={() => setSelectedCats([])} className="text-[11px] text-muted-foreground underline">
          ...
          {/* Selected chips */}
          {selectedCats.length ? (<div className="flex flex-wrap gap-1.5">
              {selectedCats.map((id) => {
                const c = CATEGORIES_BY_ID[id];
                if (!c)
                    return null;
                return (<button key={id} onClick={() => setSelectedCats((p) => p.filter((x) => x !== id))} className="rounded-full bg-primary px-2.5 py-1 text-[11px] text-primary-foreground">
                    {c.name} ✕
                  </button>);
            })}
            </div>) : null}
          ...
          <div className="-mx-1 flex gap-1 overflow-x-auto pb-1">
            {CATEGORY_GROUPS.map((g) => (<button key={g.id} onClick={() => setActiveGroup(g.id)} className={cn("shrink-0 rounded-full border px-3 py-1 text-[11px]", activeGroup === g.id
          ...
          <div className="flex flex-wrap gap-1.5">
            {(catSearch.trim()
            ? OUTING_CATEGORIES.filter((c) => c.name.toLowerCase().includes(catSearch.trim().toLowerCase()))
            : categoriesInGroup(activeGroup)).map((c) => {
            const on = selectedCats.includes(c.id);
            return (<button key={c.id} onClick={() => setSelectedCats((p) => (on ? p.filter((x) => x !== c.id) : [...p, c.id]))} className={cn("rounded-full border px-2.5 py-1 text-[11px] transition", on
                    ? "border-primary bg-primary text-primary-foreground"
                    : "bg-card hover:bg-accent")} title={c.description}>
                  {c.name}
                </button>);
        })}
          </div>
        ...
        <Card className="p-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {VIBES.map((v) => (<button key={v.id} onClick={() => setVibe(v)} className={cn("rounded-full border px-3 py-1.5 text-xs transition", vibe?.id === v.id
                ? "border-primary bg-primary text-primary-foreground"
                : "bg-card hover:bg-accent")}>
          ...
          <Input className="mt-3" placeholder="Custom vibe (optional)…" value={customVibe} onChange={(e) => setCustomVibe(e.target.value)}/>
        </Card>
        ...
        <Card className="space-y-5 p-4">
          <div>
            ...
            <Slider min={15} max={400} step={5} value={[budget]} onValueChange={([v]) => setBudget(v ?? 75)} className="mt-2"/>
          </div>
          ...
          <div>
            ...
            <Slider min={1} max={20} step={1} value={[groupSize]} onValueChange={([v]) => setGroupSize(v ?? 4)} className="mt-2"/>
          </div>
          ...
          <div>
            ...
            <Slider min={1} max={5} step={1} value={[energy]} onValueChange={([v]) => setEnergy(v ?? 3)} className="mt-2"/>
          </div>

          {/* Waterfront / scenic toggle — only when city has waterfront */}
          {waterfront?.hasWaterfront ? (<div>
              <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Waterfront
              </div>
              <div className="mt-2 flex gap-2">
                {(["auto", "yes", "no"] as const).map((opt) => (<button key={opt} onClick={() => setWaterfrontPref(opt)} className={cn("flex-1 rounded-full border px-3 py-1.5 text-xs capitalize", waterfrontPref === opt
                    ? "border-primary bg-primary text-primary-foreground"
                    : "bg-card hover:bg-accent")}>
                    {opt === "yes" ? "On the water" : opt === "no" ? "Skyline / scenic" : "Auto"}
                  </button>))}
              </div>
            </div>) : city ? (<p className="text-xs text-muted-foreground">
        ...
        ].map((f) => (<label key={f.id} className="flex items-center justify-between gap-3 text-sm">
              <span>{f.label}</span>
              <input type="checkbox" checked={f.state} onChange={(e) => f.set(e.target.checked)} className="h-4 w-4"/>
            </label>))}
          {/* Brunch Baddies-only toggle */}
          {selectedCats.includes("brunch_baddies") ||
            (vibe && BRUNCH_BADDIES_TRIGGER_IDS.has(vibe.id)) ? (<label className="flex items-center justify-between gap-3 border-t pt-2 text-sm">
              <span>🪩 Add post-brunch day party</span>
              <input type="checkbox" checked={postBrunchDayParty} onChange={(e) => setPostBrunchDayParty(e.target.checked)} className="h-4 w-4"/>
            </label>) : null}
        </Card>
        ...
        <Card className="space-y-3 p-4">
          ...
          {waterfront?.hasWaterfront ? (<label className="flex items-center justify-between gap-3 text-sm">
              <span>🛥️ Include yacht or boat option</span>
              <input type="checkbox" checked={includeYacht} onChange={(e) => setIncludeYacht(e.target.checked)} className="h-4 w-4"/>
            </label>) : null}

          <label className="flex items-start justify-between gap-3 text-sm">
            <span>
              🎰 Include casino stop
              {includeCasino ? (<span className="ml-1 block text-[10px] text-muted-foreground">
                  Play for fun — set a buffer before you sit down.
                </span>) : null}
            </span>
            <input type="checkbox" checked={includeCasino} onChange={(e) => setIncludeCasino(e.target.checked)} className="mt-1 h-4 w-4"/>
          </label>

          {vibe && ADULT_TRIGGER_IDS.has(vibe.id) ? (<div className="rounded-lg border border-dashed p-3">
              <label className="flex items-center justify-between gap-3 text-sm">
                <span>🍸 Include optional adult entertainment</span>
                <input type="checkbox" checked={includeAdult} onChange={(e) => setIncludeAdult(e.target.checked)} className="h-4 w-4"/>
              </label>
              {includeAdult ? (<label className="mt-2 flex items-start gap-2 text-xs text-muted-foreground">
                  <input type="checkbox" checked={ageConfirmed} onChange={(e) => setAgeConfirmed(e.target.checked)} className="mt-0.5 h-3.5 w-3.5"/>
                  <span>
        ...
        {error ? (<Card className="border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">
            {error}
          </Card>) : null}

        <Button size="lg" className="w-full" disabled={!city || loading} onClick={() => void build()}>
