...
export const Route = createLazyFileRoute("/viral")({
  component: ViralPage,
});
...
function ViralPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const { data } = await supabase
                .from("viral_venues")
                .select("id,city,venue_name,neighborhood,address,photo_url,rating,trend_score,tags,summary,google_place_id,source_urls,last_mentioned_at")
                .eq("city", city)
    ...
    const toggleTag = (t: ViralTag) => setActiveTags((prev) => (prev.includes(t) ? prev.filter((x) => x !== t) : [...prev, t]));
    return (<PullToRefresh onRefresh={() => setRefreshKey((k) => k + 1)}>
      <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-border bg-card p-3">
          <select value={city} onChange={(e) => setCity(e.target.value)} className="rounded-xl border border-border bg-background px-3 py-2 text-sm font-semibold">
            {CITIES.map((c) => (<option key={c} value={c}>
                {c}
              </option>))}
          </select>
          <div className="flex gap-1 rounded-xl border border-border bg-background p-1 text-xs">
            <button type="button" onClick={() => setSortBy("score")} className={`rounded-lg px-3 py-1.5 font-semibold ${sortBy === "score" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>
              Top trending
            </button>
            <button type="button" onClick={() => setSortBy("recent")} className={`rounded-lg px-3 py-1.5 font-semibold ${sortBy === "recent" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>
              Most recent
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {ALL_VIRAL_TAGS.map((t) => {
            ...
            return (<button key={t} type="button" onClick={() => toggleTag(t)} className={`transition ${active ? "scale-105" : "opacity-70 hover:opacity-100"}`} aria-pressed={active}>
                <ViralTagChip tag={t} size="md"/>
              </button>);
        })}
          {activeTags.length > 0 && (<button type="button" onClick={() => setActiveTags([])} className="rounded-full border border-border px-2.5 py-1 text-[11px] font-semibold text-muted-foreground hover:text-foreground">
              Clear
            </button>)}
        </div>

        {filtered === null && (<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[...Array(6)].map((_, i) => (<div key={i} className="h-72 animate-pulse rounded-2xl bg-muted"/>))}
          </div>)}

        {filtered !== null && filtered.length === 0 && (<div className="rounded-2xl border border-dashed border-border bg-card/50 p-10 text-center">
        ...
        {filtered && filtered.length > 0 && (<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((v) => (<ViralBigCard key={v.id} v={v}/>))}
          </div>)}
      </div>
    </PullToRefresh>);
}

function ViralBigCard({ v }: {
    v: Row;
}) {
    ...
    return (<article className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
      ...
      <div className="space-y-2 p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          {typeof v.rating === "number" && (<span className="flex shrink-0 items-center gap-1 text-xs font-semibold text-amber-600">
              <Star className="h-3 w-3 fill-amber-500 text-amber-500"/>
              {v.rating.toFixed(1)}
            </span>)}
        </div>
        {v.neighborhood && (<p className="flex items-center gap-1 text-xs text-muted-foreground">
        ...
        <div className="flex flex-wrap gap-1">
          {(v.tags ?? []).map((t) => (<ViralTagChip key={t} tag={t}/>))}
        </div>
        <div className="flex items-center justify-between pt-1">
          <a href={mapHref} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-semibold text-primary hover:underline">
